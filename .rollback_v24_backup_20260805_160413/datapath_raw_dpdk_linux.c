...
/* GREENQUIC-V24-POLICY-WINDOW-FIX: evaluate policy by time, not poll count. */
#define DEFAULT_GREENQUIC_POLICY_PERIOD_US        50U
#define DEFAULT_GREENQUIC_NO_SLEEP_ACTIVE_HOLD_US 1000U
...
typedef struct GREENQUIC_LCORE_STATE {
    ...
    /* GREENQUIC-V24-POLICY-WINDOW-FIX */
    uint64_t LastPolicyTsc;
    uint32_t WindowRxBurstMax;
    uint32_t WindowRxQueueMax;
    uint32_t WindowTxBurstMax;
    uint32_t WindowTxRingMax;
    ...
    uint64_t TotalCStateRequestedUs;
...
// GREENQUIC-END
static const char GreenQuicEpollWorkWaitMarker[] __attribute__((used)) = "epoll_work_wait";
...
typedef struct DPDK_DATAPATH {
    ...
    /* GREENQUIC-V24-POLICY-WINDOW-FIX */
    uint32_t GreenQuicPolicyPeriodUs;     // minimum interval between full policy evaluations
    uint32_t GreenQuicNoSleepActiveHoldUs; // recent physical work forces max when sleep is disabled
    ...
    )
{
    ...
    Dpdk->GreenQuicPolicyPeriodUs = DEFAULT_GREENQUIC_POLICY_PERIOD_US;
    Dpdk->GreenQuicNoSleepActiveHoldUs =
        DEFAULT_GREENQUIC_NO_SLEEP_ACTIVE_HOLD_US;
...
// GREENQUIC-V22-CPU-FREQ-LOGGING-HOTFIX
...
#undef GQ_CLAMP_ALPHA
     ...
     *   GREENQUIC_POLICY_PERIOD_US=<uint32>
     *   GREENQUIC_NO_SLEEP_ACTIVE_HOLD_US=<uint32>
     */
    ...
    /* GREENQUIC-V24-POLICY-WINDOW-FIX direct runtime controls. */
    const char* PolicyPeriodEnv = getenv("GREENQUIC_POLICY_PERIOD_US");
    if (PolicyPeriodEnv != NULL && PolicyPeriodEnv[0] != '\0') {
        errno = 0;
        char* End = NULL;
        const unsigned long Value = strtoul(PolicyPeriodEnv, &End, 10);
        if (errno == 0 && End != PolicyPeriodEnv && *End == '\0' &&
            Value <= UINT32_MAX) {
            Dpdk->GreenQuicPolicyPeriodUs = (uint32_t)Value;
        } else {
            fprintf(
                stderr,
                "GreenQUIC: invalid GREENQUIC_POLICY_PERIOD_US=%s; keeping %u.\n",
                PolicyPeriodEnv,
                Dpdk->GreenQuicPolicyPeriodUs);
        }
    }

    const char* ActiveHoldEnv = getenv("GREENQUIC_NO_SLEEP_ACTIVE_HOLD_US");
    if (ActiveHoldEnv != NULL && ActiveHoldEnv[0] != '\0') {
        errno = 0;
        char* End = NULL;
        const unsigned long Value = strtoul(ActiveHoldEnv, &End, 10);
        if (errno == 0 && End != ActiveHoldEnv && *End == '\0' &&
            Value <= UINT32_MAX) {
            Dpdk->GreenQuicNoSleepActiveHoldUs = (uint32_t)Value;
        } else {
            fprintf(
                stderr,
                "GreenQUIC: invalid GREENQUIC_NO_SLEEP_ACTIVE_HOLD_US=%s; keeping %u.\n",
                ActiveHoldEnv,
                Dpdk->GreenQuicNoSleepActiveHoldUs);
        }
    }
    ...
    )
{
    ...
    if (AddressRet != 0) {
        fprintf(
            stderr,
            "[CPU %u] GreenQUIC IDLE mode=monitor result=unavailable port=%hu queue=%hu "
    ...
    if (Ret == -ENOTSUP || Ret == -EINVAL) {
        fprintf(
            stderr,
            "[CPU %u] GreenQUIC IDLE mode=monitor result=disabled rte_power_monitor ret=%d "
            "(%s). Falling back.\n\n",
            Core,
            Ret,
            rte_strerror(-Ret));
    ...
    )
{
    ...
    if (S->EpollFd < 0) {
        ...
        fprintf(
            stderr,
            "[CPU %u] GreenQUIC IDLE mode=epoll result=unavailable epoll_create1 failed: "
    ...
    if (S->WakeEventFd < 0) {
        ...
        fprintf(
            stderr,
            "[CPU %u] GreenQUIC IDLE mode=epoll result=unavailable eventfd failed: "
            "errno=%d (%s). Falling back.\n\n",
            Core,
            SavedErrno,
            strerror(SavedErrno));
    ...
    if (epoll_ctl(S->EpollFd, EPOLL_CTL_ADD, S->WakeEventFd, &Ev) != 0) {
        ...
        fprintf(
            stderr,
            "[CPU %u] GreenQUIC IDLE mode=epoll result=unavailable eventfd epoll_ctl failed: "
            "errno=%d (%s). Falling back.\n\n",
            Core,
            SavedErrno,
            strerror(SavedErrno));
    ...
    )
{
    ...
    if (OwnsRx) {
        ...
        if (RxFd < 0) {
            ...
            fprintf(
                stderr,
                "[CPU %u] GreenQUIC IDLE mode=epoll result=unavailable port=%hu queue=%hu "
    ...
    if (RxInterruptArmed) {
        ...
        if (DisableRet != 0) {
            fprintf(
                stderr,
                "[CPU %u] GreenQUIC IDLE mode=epoll result=warning port=%hu queue=%hu "
    ...
    fprintf(
        stderr,
        "[CPU %u] GreenQUIC IDLE mode=epoll result=disabled epoll_wait errno=%d (%s). "
        "Falling back.\n\n",
        Core,
        WaitErrno,
        strerror(WaitErrno));
    ...
    )
{
    ...
    printf(
        ...
        "policy_period_us=%u no_sleep_active_hold_us=%u "
    ...
    )
{
    ...
    if (Ret == -ENOTSUP || Ret == -EINVAL) {
        fprintf(
            stderr,
            "[CPU %u] GreenQUIC IDLE mode=pause result=disabled rte_power_pause ret=%d (%s). "
            "Falling back.\n\n",
            Core,
            Ret,
            rte_strerror(-Ret));
    ...
    )
{
    ...
    GREENQUIC_LCORE_STATE* S = GreenQuicGetLcoreState(Dpdk, Core);
    GREENQUIC_DIR_STATE* Rx = &S->Rx;
    Rx->Polls++;
    Rx->LastBurstCount = BuffersCount;
    Rx->LastQueueCount = RxQueueCountBefore > 0 ? (uint32_t)RxQueueCountBefore : BuffersCount;
    /* GREENQUIC-V24-POLICY-WINDOW-FIX: preserve work seen anywhere in the window. */
    S->WindowRxBurstMax = GreenQuicMaxU32(
        S->WindowRxBurstMax, BuffersCount);
    S->WindowRxQueueMax = GreenQuicMaxU32(
        S->WindowRxQueueMax, Rx->LastQueueCount);
    ...
    if (BuffersCount == Dpdk->RxBurstSize) {
        Rx->FullBursts++;
        Rx->ConsecutiveFull++;
    } else {
        Rx->ConsecutiveFull = 0;
    ...
    )
{
    ...
    GREENQUIC_LCORE_STATE* S = GreenQuicGetLcoreState(Dpdk, Core);
    GREENQUIC_DIR_STATE* Tx = &S->Tx;
    Tx->Polls++;
    Tx->LastQueueCount = RingBefore;
    Tx->LastBurstCount = BufferCount;
    /* GREENQUIC-V24-POLICY-WINDOW-FIX: preserve work seen anywhere in the window. */
    S->WindowTxBurstMax = GreenQuicMaxU32(
        S->WindowTxBurstMax, BufferCount);
    S->WindowTxRingMax = GreenQuicMaxU32(
        S->WindowTxRingMax, RingBefore);
    ...
    Tx->Packets += TxCount;
    if (BufferCount == Dpdk->TxBurstSize) {
        Tx->FullBursts++;
        Tx->ConsecutiveFull++;
    } else {
        Tx->ConsecutiveFull = 0;
    ...
    )
{
    ...
    /* GREENQUIC-V24-POLICY-WINDOW-FIX: one empty poll must not erase work
     * observed earlier in the fixed policy window. */
    const uint32_t RxBurstSample = GreenQuicMaxU32(
        S->Rx.LastBurstCount, S->WindowRxBurstMax);
    const uint32_t RxQueueSample = GreenQuicMaxU32(
        S->Rx.LastQueueCount, S->WindowRxQueueMax);
    const uint32_t TxBurstSample = GreenQuicMaxU32(
        S->Tx.LastBurstCount, S->WindowTxBurstMax);
    const uint32_t TxRingSample = GreenQuicMaxU32(
        GreenQuicMaxU32(TxRingCount, S->Tx.LastQueueCount),
        S->WindowTxRingMax);

    S->LastRxBurstPressure = OwnsRx ?
        GreenQuicPressureFromRatioConfigured(
            Dpdk, RxBurstSample, Dpdk->RxBurstSize) : 0U;
    S->LastRxQueuePressure = OwnsRx ?
        GreenQuicPressureFromRatioConfigured(
            Dpdk, RxQueueSample, Dpdk->GreenQuicRxQueueHigh) : 0U;
    S->LastTxBurstPressure = OwnsTx ?
        GreenQuicPressureFromRatioConfigured(
            Dpdk, TxBurstSample, Dpdk->TxBurstSize) : 0U;

    const uint32_t TxRingNowPressure = OwnsTx ?
        GreenQuicPressureFromRatioConfigured(
            Dpdk, TxRingSample, Dpdk->GreenQuicTxRingHigh) : 0U;
    const uint32_t TxRingLastPressure = TxRingNowPressure;
    ...
    )
{
    ...
    const uint64_t IdleUs = LastActive == 0U ? UINT64_MAX :
        GreenQuicTscDeltaToUs(Now - LastActive);
    const BOOLEAN NoSleepRecentWork =
        !Dpdk->GreenQuicEnableSleep &&
        Dpdk->GreenQuicNoSleepActiveHoldUs != 0U &&
        IdleUs < Dpdk->GreenQuicNoSleepActiveHoldUs;

    /* GREENQUIC-V24-POLICY-WINDOW-FIX:
     * - sleep=0 never pauses; recent physical work immediately protects max.
     * - the expensive policy and its EWMAs run on a fixed time cadence rather
     *   than once per empty poll. This removes poll-rate-dependent decay. */
    if (NoSleepRecentWork && Dpdk->GreenQuicEnableFreq && !S->FreqIsMax) {
        S->LastAction = "active_poll_max";
        S->LastPolicyTsc = Now;
        GreenQuicFreqMax(Dpdk, Core);
        return;
    }

    if (Dpdk->GreenQuicPolicyPeriodUs != 0U &&
        S->LastPolicyTsc != 0U &&
        GreenQuicTscDeltaToUs(Now - S->LastPolicyTsc) <
            Dpdk->GreenQuicPolicyPeriodUs) {
        S->LastAction = NoSleepRecentWork ?
            "active_poll_hold" : "policy_window_poll";
        return;
    }
    S->LastPolicyTsc = Now;
    ...
    const uint32_t TxRingCount = OwnsTx ?
        rte_ring_count(Interface->TxRingBuffer) : 0U;
    S->WindowTxRingMax = GreenQuicMaxU32(
        S->WindowTxRingMax, TxRingCount);

    BOOLEAN HardMax = FALSE;
    (void)GreenQuicComputeRawPressure(
        Dpdk, S, Core, TxRingCount, &HardMax);

    S->WindowRxBurstMax = 0U;
    S->WindowRxQueueMax = 0U;
    S->WindowTxBurstMax = 0U;
    S->WindowTxRingMax = 0U;
    ...
    /* IdleUs was computed once at the beginning of this policy window. */
...
)
{
    ...
    /*
     * GREENQUIC-V22-ARGV-DEDUP-CLEANUP
     *
     * Some generated EAL argv layouts may contain duplicate pointers or an
     * entry that aliases the argv allocation itself. Free every distinct
     * argument allocation at most once, then free the argv array once.
     */
    if (Dpdk->DpdkInitArgv != NULL) {
        char** InitArgv = Dpdk->DpdkInitArgv;
        const int InitArgc = Dpdk->DpdkInitArgc;

        for (int i = 0; i < InitArgc; ++i) {
            char* Arg = InitArgv[i];

            if (Arg == NULL || (void*)Arg == (void*)InitArgv) {
                continue;
            }
