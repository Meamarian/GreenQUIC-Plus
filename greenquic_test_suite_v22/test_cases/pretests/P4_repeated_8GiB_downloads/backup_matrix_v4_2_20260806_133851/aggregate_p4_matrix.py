...
def read_runs(folder: Path, role: str) -> list[dict[str, Any]]:
    ...
    for path in sorted(folder.glob(f"{role}_rep*_*.log")):
        ...
        payload_gib = extract_number(rows.get("greenquic_p4_workload_summary__total_payload"))
...
def build_combined_rows(
    ...
    for rep, mode in keys:
        ...
        payload_gib = extract_number(client_log.get("greenquic_p4_workload_summary__total_payload"))
        start_skew_ms = None
        end_skew_ms = None
        if server_energy.get("start_wall_time_ns") and client_energy.get("start_wall_time_ns"):
            start_skew_ms = abs(int(server_energy["start_wall_time_ns"]) - int(client_energy["start_wall_time_ns"])) / 1e6
        if server_energy.get("end_wall_time_ns") and client_energy.get("end_wall_time_ns"):
            end_skew_ms = abs(int(server_energy["end_wall_time_ns"]) - int(client_energy["end_wall_time_ns"])) / 1e6
        result.append({
            "repetition": rep,
            "mode": mode,
            "workload_duration_s": extract_number(client_log.get("greenquic_p4_workload_summary__workload_elapsed_time_including_gaps")),
            "goodput_excluding_gaps_gbps": extract_number(client_log.get("greenquic_p4_workload_summary__aggregate_goodput_excluding_gaps")),
            "goodput_including_gaps_gbps": extract_number(client_log.get("greenquic_p4_workload_summary__aggregate_goodput_including_gaps")),
...
def average_combined(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    numeric_fields = [
        ...
        "window_start_skew_ms", "window_end_skew_ms",
    ]
    averages: list[dict[str, Any]] = []
    for mode in MODES:
        mode_rows = [row for row in rows if row["mode"] == mode]
        avg: dict[str, Any] = {"mode": mode, "valid_runs": len(mode_rows)}
...
def create_charts(output: Path, averages: list[dict[str, Any]]) -> list[Path]:
    try:
        ...
        print(f"[P4:WARN] SVG chart generation skipped: {exc}")
        return []
...
def main() -> int:
    ...
    combined_runs = build_combined_rows(role_runs.get("client", []), aligned)
    ...
    combined_columns = [
        ...
        "client_aligned_valid", "server_aligned_valid", "client_aligned_file", "server_aligned_file",
    ]
    write_csv(output / "combined_endpoint_all_runs.csv", combined_runs, combined_columns)

    averages = average_combined(combined_runs)
    average_columns = ["mode", "valid_runs"] + [
        key for key in averages[0] if key not in {"mode", "valid_runs"}
    ] if averages else ["mode", "valid_runs"]
    ...
    print()
    print("=== P4 aligned server + client comparison ===")
    print(comparison)
    print()
    if combined_runs:
        ...
        if max_start_skew > 250 or max_end_skew > 250:
            print("- WARNING: endpoint window skew exceeded 250 ms; inspect aligned JSON files before publication.")
    else:
