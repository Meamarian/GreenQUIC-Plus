...
ssh "root@$CLIENT_HOST" "test -x $(quote "$CLIENT_BIN_EFFECTIVE")" || {
...
if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
    ...
    ssh "root@$CLIENT_HOST" "test -x $(quote "$CLIENT_DIR/rapl_window.py")" || {
...
while IFS=$'\t' read -r TEST_INDEX rep position mode; do
    [[ "$TEST_INDEX" == test_index ]] && continue
    ...
    if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
        ...
        if ! ssh "root@$CLIENT_HOST" "cat $(quote "$client_remote_result")" > "$client_aligned_result"; then
            warn "failed to retrieve client aligned RAPL JSON for $run_id"
            rm -f "$client_aligned_result"
        fi
        ssh "root@$CLIENT_HOST" "rm -f $(quote "$client_remote_state") $(quote "$client_remote_result")" || true
    ...
    notice "TEST $(printf '%02d' "$TEST_INDEX")/$TOTAL_TESTS mode=$mode COMPLETE"
    [[ "$TEST_INDEX" -eq "$TOTAL_TESTS" ]] || sleep "$BETWEEN_TESTS_SECONDS"
done < "$SCHEDULE"

python3 "$HERE/aggregate_p4_matrix.py" --input "$OUTPUT_DIR" --runs "$RUNS"

echo
notice "SUCCESS: all $TOTAL_TESTS P4 workloads completed"
