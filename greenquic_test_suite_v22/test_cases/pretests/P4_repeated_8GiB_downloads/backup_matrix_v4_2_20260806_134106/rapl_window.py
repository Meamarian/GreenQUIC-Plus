...
def discover_domains() -> list[dict[str, Any]]:
    ...
    # /sys/class/powercap/intel-rapl:* entries are symlinked directories.
    # pathlib.rglob() does not recurse through those class symlinks, which made
    # the earlier helper incorrectly report zero domains while the suite's C
    # sampler could read package and DRAM counters. os.walk(..., followlinks=True)
    # follows the class links; resolved paths are deduplicated below.
    seen: set[str] = set()
    for dirpath, dirnames, filenames in os.walk(POWER_ROOT, followlinks=True):
