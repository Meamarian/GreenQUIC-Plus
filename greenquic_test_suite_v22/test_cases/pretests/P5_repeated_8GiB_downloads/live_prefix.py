...
def main() -> int:
    ...
    request_start = re.compile(
        r"\[GreenQUIC-P5\] request=(\d+)/(\d+) start_us="
    )
    request_complete = re.compile(
        r"\[GreenQUIC-P5\] request=(\d+)/(\d+) complete_us="
    )
    gap = re.compile(r"\[GreenQUIC-P5\] gap_after=(\d+)")
    get_count = 0
    ...
    for raw in sys.stdin:
        ...
        print(
            f"[{wall_timestamp()}][+{elapsed:09.3f}s]{base}{suffix} {line}",
            flush=True,
        )
    return 0
