...
make_remote_manifest() {
    local remote="$1"
    retry_ssh "cd '$remote' && rm -f SHA256SUMS.tmp && find . -maxdepth 1 -type f ! -name SHA256SUMS ! -name SHA256SUMS.tmp -printf '%f\\0' | sort -z | xargs -0 -r sha256sum > SHA256SUMS.tmp && mv SHA256SUMS.tmp SHA256SUMS"
}
...
copy_export_verified() {
    ...
    log "Final hash verification failed; retrying export in 30 s"
    sleep 30
...
verify_previous_export_if_any() {
    local prev="" localdir
    while true; do
        prev="$(ssh "${SSH_OPTS[@]}" idex 'for d in /tmp/P5_RESUME_EXPORT_* /tmp/P5_P7_FINAL_EXPORT_*; do [ -d "$d" ] && printf "%s %s\n" "$(stat -c %Y "$d" 2>/dev/null || echo 0)" "$d"; done | sort -nr | head -1 | cut -d" " -f2-' 2>/dev/null)"
        rc=$?
        [ "$rc" -eq 0 ] && break
        log "Cannot query previous export yet; retrying in 30 s"
        sleep 30
    done
    if [ -z "$prev" ]; then
        log "No previous P5 export directory found"
        return 0
    fi
    localdir="$HOME/Downloads/$(basename "$prev")"
    log "Verifying/copying latest previous export before new testing: $prev"
...
sync_branch_both() {
    ...
    retry_ssh "ssh -o ConnectTimeout=15 -o ServerAliveInterval=20 -o ServerAliveCountMax=3 root@tinyman 'cd /root/mohsen && git reset --hard && git fetch \"$REMOTE_BUNDLE\" \"refs/heads/$tempref\" && git checkout -B \"$branch\" FETCH_HEAD && test \"\$(git rev-parse HEAD)\" = \"$expected\"'"
}
...
wait_for_export_done() {
    local export="$1" label="$2"
    while true; do
        if ssh "${SSH_OPTS[@]}" idex "test -f '$export/DONE'" >/dev/null 2>&1; then
            return 0
        fi
        log "Waiting for $label; Mac-idex SSH failures do not stop the remote nohup job"
        sleep 60
    done
}
...
log "Order: clean+verify old SCP -> finish P1 -> SCP/hash -> 5 min -> P2 best 6x5 -> SCP/hash -> 5 min -> 12-profile P2 1x3 -> SCP/hash"
...
wait_clean_both
log "Performance1 is finished and SHA256-verified on the Mac"
log "Cooling down 5 minutes"
sleep 300
...
wait_for_export_done "$P2_BEST_EXPORT" "Performance2 $P2_BEST_PROFILE 6x5"
copy_export_verified "$P2_BEST_EXPORT" "$P2_BEST_LOCAL"
wait_clean_both
log "Performance2 best-profile 6x5 is finished and SHA256-verified on the Mac"
log "Cooling down 5 minutes before 12-profile sweep"
sleep 300
...
wait_for_export_done "$P2_SWEEP_EXPORT" "Performance2 12-profile 1x3 sweep"
...
log "P2 12-profile 1x3: $P2_SWEEP_LOCAL"
log "All exports SHA256-verified; idex and tinyman clean."
log "======================================================================"
