...
validate_runtime_log() {
    ...
    # GREENQUIC-COUNTERS-V1: independent of GQ_LOG_LEVEL.
    # This mandatory process-end counter parser runs for recorded OFF/BASIC/PLUS
    # workloads even when periodic GreenQUIC decision logging is disabled.
    if gq_recording_enabled; then
        python3 "$GQ_COMMON_DIR/bin/parse_greenquic_counters.py" "$logf" \
            --mode "$mode" \
            --out "$TEST_DIR/results/${role}_${mode}_${stamp}_greenquic_counters.csv" || return $?
    fi
