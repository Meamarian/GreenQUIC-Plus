...
# Performance1 mirror of the final Performance2 validation.
# The comparison intentionally keeps the workload, GreenQUIC runtime profiles,
# P7 Linux PAPER_FULL configuration, recording, reporting, and export pipeline
# identical. The only intended P5 datapath change is Performance1 vs Performance2.
#
# P5:
#   1) IDLE_MONITOR_NORMAL: OFF/BASIC/PLUS
#   2) POWER_FRIENDLY:     OFF/BASIC/PLUS
#   runs/downloads default to 6x6
#
# P7 PAPER_FULL:
...
[[ -n "$REPO_ROOT" && -d "$REPO_ROOT/.git" ]] || {
    echo "ERROR: run from GreenQUIC repo or set GREENQUIC_REPO" >&2
    exit 2
}
...
LOCAL_LOCK="$HOME/Downloads/.greenquic_p1_final_6x6_p7.lock"
BUNDLE="${TMPDIR:-/tmp}/GreenQUIC_P1_FINAL_${TAG}.bundle"
REMOTE_BUNDLE="/tmp/GreenQUIC_P1_FINAL_${TAG}.bundle"
REMOTE_SCRIPT_LOCAL="${TMPDIR:-/tmp}/P5_P1_FINAL_REMOTE_${TAG}_$$.sh"
REMOTE_SCRIPT="/tmp/P5_P1_FINAL_REMOTE_${TAG}.sh"
REMOTE_LOG="/root/P5_P1_FINAL_${TAG}.log"
REMOTE_PID="/tmp/P5_P1_FINAL_${TAG}.pid"
EXPORT_REMOTE="/tmp/P5_P1_FINAL_EXPORT_${TAG}"
EXPORT_LOCAL="$HOME/Downloads/P5_P1_FINAL_EXPORT_${TAG}"
...
if [[ "${1:-}" == "--detach" ]]; then
    shift
    LOG="$HOME/Downloads/P5_P1_FINAL_${TAG}.mac.log"
    PIDFILE="$HOME/Downloads/P5_P1_FINAL_${TAG}.mac.pid"
    if [[ -f "$PIDFILE" ]]; then
        old="$(cat "$PIDFILE" 2>/dev/null || true)"
        if [[ -n "$old" ]] && kill -0 "$old" 2>/dev/null; then
            echo "ERROR: final P1/P7 runner already alive PID=$old" >&2
            exit 70
        fi
    fi
    ...
    echo "STARTED P1 FINAL 6x6 + P7 PAPER PID=$pid"
    echo "TAG=$TAG"
    echo "LOG=$LOG"
...
if [[ -d "$LOCAL_LOCK" ]]; then
    ...
    if [[ -n "$old" ]] && kill -0 "$old" 2>/dev/null; then
        echo "ERROR: another Performance1 final validation runner is alive PID=$old" >&2
        exit 70
    fi
    rm -rf "$LOCAL_LOCK"
fi
...
# Refuse local overlap with active final/test orchestrators from either branch.
for pf in \
    "$HOME/Downloads"/P5_P1_FINAL_*.mac.pid \
    "$HOME/Downloads"/P5_P2_GOODPUT_*.mac.pid \
    "$HOME/Downloads"/P5_P2_PLUS_IDLE_*.mac.pid \
    "$HOME/Downloads"/P5_P2_FINAL_*.mac.pid; do
    [[ -f "$pf" ]] || continue
    [[ "$pf" == "$HOME/Downloads/P5_P1_FINAL_${TAG}.mac.pid" ]] && continue
...
for p in /proc/[0-9]*; do
    ...
    case "$cmd" in *run_matrix_with_sheet.sh*|*run_matrix_with_report.sh*|*run_matrix_from_idex.sh*|*run_client.sh*|*run_server.sh*|*P5_P1_FINAL_REMOTE*|*P5_P2_FINAL_REMOTE*) bad=1;; esac
done
...
fetch_bundle(){
    ...
    REF="refs/heads/__p1_final_${TAG}_$$"
    ...
    while true; do
        ...
        if (( rc == 255 )); then
            log "idex -> tinyman bundle SCP transport failed; retrying in 30 s"
            sleep 30
            continue
        fi
...
MON="$P5/matrix_results/P5_P1_FINAL_idle_monitor_${RUNS}r_${DOWNLOADS}d_${TAG}"
PWR="$P5/matrix_results/P5_P1_FINAL_power_friendly_${RUNS}r_${DOWNLOADS}d_${TAG}"
P7OUT="$P7/matrix_results/P7_P1_FINAL_linux_paper_${RUNS}r_${DOWNLOADS}d_${TAG}"
EX="/tmp/P5_P1_FINAL_EXPORT_${TAG}"
mkdir -p "$EX"
...
P5_MARKER='GREENQUIC-P5-SUPER-PERF-V2 cache=128 rxb=32 txb=16 ring=4096 sync=legacy drain=2 threshold=0 mtu=0 skipoffcount=0 debugcounters=1 transferwindow=1 traceringcount=1 txmeta=mbuf rxmeta=mbuf txlock=single_owner capdiag=1'
...
# Build the exact promoted Performance1 datapath on both endpoints.
echo '=== BUILD P5 PROMOTED PERFORMANCE1 DEFAULT ==='
(cd "$P5" && env "${PERF_ENV[@]}" bash ./build_p5_super_performance.sh >"$EX/build_p5_idex.log" 2>&1) & p1=$!
ssh -n root@tinyman "cd '$P5' && env ${PERF_ENV[*]} bash ./build_p5_super_performance.sh" >"$EX/build_p5_tinyman.log" 2>&1 & p2=$!
wait "$p1"; BP51=$?
wait "$p2"; BP52=$?
...
VP51=1; VP52=1
if [ "$BP51" -eq 0 ]; then
    grep -aFq -- "$P5_MARKER" "$ROOT/msquic/build-greenquic-p5/bin/Release/quicinterop"; VP51=$?
fi
if [ "$BP52" -eq 0 ]; then
    ssh -n root@tinyman "grep -aFq -- '$P5_MARKER' '$ROOT/msquic/build-greenquic-p5/bin/Release/quicinterop'"; VP52=$?
fi
echo "P5 promoted-marker verify: idex=$VP51 tinyman=$VP52"

# Build the isolated Linux P7 binaries on both endpoints before measurement.
echo '=== BUILD P7 LINUX ==='
...
if [ "$BP51" -eq 0 ] && [ "$BP52" -eq 0 ] && [ "$VP51" -eq 0 ] && [ "$VP52" -eq 0 ]; then
    ...
    echo 'ERROR: promoted Performance1 P5 build/marker verification failed; P5 matrices skipped.'
fi
...
zip_one "$MON" "P5_P1_IDLE_MONITOR_${RUNS}r_${DOWNLOADS}d_${TAG}"
zip_one "$PWR" "P5_P1_POWER_FRIENDLY_${RUNS}r_${DOWNLOADS}d_${TAG}"
...
wait_remote_done(){
    while true; do
        if ssh_once "test -f '$EXPORT_REMOTE/DONE'" >/dev/null 2>&1; then
            log "remote final Performance1 P5/P7 suite DONE detected"
            return 0
        fi
        ...
        log "waiting for final Performance1 6x6 P5/P7 suite (Mac->idex SSH failure cannot kill remote nohup job)"
        sleep 60
    done
}
...
log "P1 FINAL startup tag=$TAG runs=$RUNS downloads=$DOWNLOADS seed=$SEED"
wait_clean_both
sleep "$START_DELAY"
fetch_bundle
sync_branch
...
ssh_transport_retry "cd '/root/mohsen/$P5_REL' && bash -n ./build_p5_super_performance.sh ./run_matrix_with_sheet.sh && python3 -m py_compile ./apply_p5_super_performance.py ./apply_p5_super_packet_counter_guard.py && cd '/root/mohsen/$P7_REL' && bash -n ./build_p7_linux.sh ./run_matrix_with_report.sh ./run_matrix_from_idex.sh"
ssh_transport_retry "ssh -o ConnectTimeout=15 root@tinyman 'cd /root/mohsen/$P5_REL && bash -n ./build_p5_super_performance.sh ./run_matrix_with_sheet.sh && python3 -m py_compile ./apply_p5_super_performance.py ./apply_p5_super_packet_counter_guard.py && cd /root/mohsen/$P7_REL && bash -n ./build_p7_linux.sh ./run_matrix_with_report.sh ./run_matrix_from_idex.sh'"
