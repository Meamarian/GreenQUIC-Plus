...
def main()->int:
    ...
    for rep in range(1,a.runs+1):
        for mode in MODES:
            ...
            detail={'schema':'greenquic-p5-parallel-active-v2','run_id':run_id,'timing_definition':'parallel batch start to parallel batch complete; client CLOCK_MONOTONIC mapped to server CLOCK_MONOTONIC with pre-run clock_sync','client_window_ns':[cstart,cend],'server_window_ns':[sstart,send],'clock_sync':{'client_minus_server_monotonic_offset_ns':offset,'uncertainty_ns':int(sync.get('monotonic_uncertainty_ns',0))},'goodput':{'aggregate_gbps':row['aggregate_goodput_gbps'],'connections':conns},'rapl':{'server':sr,'client':cr,'combined_total_j':combined_j,'combined_avg_w':combined_w,'server_j_per_useful_gbit':row['server_j_per_useful_gbit'],'client_j_per_useful_gbit':row['client_j_per_useful_gbit'],'combined_j_per_useful_gbit':row['combined_j_per_useful_gbit']},'frequency':{'server':sf,'client':cf}}
            (json_dir/f'{run_id}.json').write_text(json.dumps(detail,indent=2)+'\n');rows.append(row)
            print(f"P5 ACTIVE {run_id}: total_goodput={row['aggregate_goodput_gbps']:.6f} Gbit/s server_RAPL={sr['total_j']:.3f} J client_RAPL={cr['total_j']:.3f} J combined_RAPL={combined_j:.3f} J combined_power={combined_w:.3f} W")
    ...
    measures=['aggregate_goodput_gbps','average_individual_goodput_gbps','server_total_j','client_total_j','combined_total_j','server_avg_rapl_w','client_avg_rapl_w','combined_avg_rapl_w','server_j_per_useful_gbit','client_j_per_useful_gbit','combined_j_per_useful_gbit','server_cpu19_mean_ghz','server_cpu20_mean_ghz','server_dataplane_mean_ghz','client_cpu19_mean_ghz','client_cpu20_mean_ghz','client_dataplane_mean_ghz'];summ=[]
    for mode in MODES:
