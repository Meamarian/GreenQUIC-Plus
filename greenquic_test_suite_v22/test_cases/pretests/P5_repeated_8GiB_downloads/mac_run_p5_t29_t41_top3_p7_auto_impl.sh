...
matrix folder names: T29_<timestamp>, T41_<timestamp>, TOP3_<timestamp>, P7_<timestamp>
...
P5_T29_OUT="$P5/matrix_results/T29_${STAMP}"
...
# The previous launcher incorrectly required *_affinity.txt inside final P5
# matrix bundles. bundle_run_results.py does not currently preserve those
# temporary files, so that check caused rc=1 after a successful 18/18 matrix.
# Validate the applied affinity patch here, then validate completed bundles,
# charts and workbook after each P5 matrix instead.
...
cleanup_both
log "TEST 1/4 P5 T29 RX_QUEUE_HIGH=48 — OFF + BASIC + PLUS"
cd "$P5"
bash ./run_matrix_with_sheet.sh "${P5_COMMON_ARGS[@]}" \
    --output-dir "$P5_T29_OUT" \
    --env RX_QUEUE_HIGH=48 \
    2>&1 | tee "$ART/p5_t29.log"
validate_p5_matrix T29 "$P5_T29_OUT"

cleanup_both
sleep "$BETWEEN"
log "TEST 2/4 P5 T41 ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16 — OFF + BASIC + PLUS"
cd "$P5"
bash ./run_matrix_with_sheet.sh "${P5_COMMON_ARGS[@]}" \
    --output-dir "$P5_T41_OUT" \
    --env ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16 \
    2>&1 | tee "$ART/p5_t41.log"
validate_p5_matrix T41 "$P5_T41_OUT"

cleanup_both
sleep "$BETWEEN"
log "TEST 3/4 P5 TOP3 — OFF + BASIC + PLUS"
cd "$P5"
bash ./run_matrix_with_sheet.sh "${P5_COMMON_ARGS[@]}" \
...
validate_p5_matrix TOP3 "$P5_TOP3_OUT"

cleanup_both
sleep "$BETWEEN"
log "TEST 4/4 P7 paper Linux baseline"
cd "$P7"
...
# Affinity evidence is informative only; do not make a completed P7 experiment
# fail because an optional evidence file was not copied into the final bundle.
find "$P7_OUT/runs" -type f \( -name 'rapl_affinity.txt' -o -name 'frequency_affinity.txt' -o -name 'cstate_affinity.txt' \) -print > "$ART/p7_affinity.txt" || true
...
T29_matrix=$P5_T29_OUT
T41_matrix=$P5_T41_OUT
TOP3_matrix=$P5_TOP3_OUT
P7_matrix=$P7_OUT
CFG

log "zip all four result directories"
T29_ZIP="/root/T29_${STAMP}.zip"
...
printf '%s\n%s\n%s\n%s\n' "$T29_ZIP" "$T41_ZIP" "$TOP3_ZIP" "$P7_ZIP" > "$ART/RESULT_ZIPS.txt"
touch "$ART/DONE"
log "DONE"
...
echo "REMOTE MATRIX NAMES: T29_$STAMP  T41_$STAMP  TOP3_$STAMP  P7_$STAMP"
...
echo "Waiting for all four tests to finish; Mac must remain awake/network-connected for automatic SCP."
while :; do
