...
# First create the exact promoted Performance2 source/binary. Snapshot instrumentation is additive.
...
for bin in "$BUILD/bin/Release/quicinterop" "$BUILD/bin/Release/quicinteropserver"; do
  ...
  grep -aFq -- "$MARK" "$bin" || { echo "ERROR: D1/D2+ snapshot marker missing: $bin" >&2; exit 3; }
done
