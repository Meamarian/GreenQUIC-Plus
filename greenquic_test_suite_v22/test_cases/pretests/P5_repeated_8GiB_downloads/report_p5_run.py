...
REQUEST_RE = re.compile(
    r"^\[GreenQUIC-P5\] request=(\d+)/(\d+) "
    r"(start|complete)_us=(\d+) path=(\S+)"
    r"(?: duration_us=(\d+) success=(0|1))?$"
)
GAP_RE = re.compile(
    r"^\[GreenQUIC-P5\] gap_after=(\d+) requested_us=(\d+) actual_us=(\d+)$"
)
...
def main() -> int:
    ...
    if len(starts) != expected or len(completions) != expected:
        raise SystemExit(
            "ERROR: P5 markers are incomplete: "
            f"starts={len(starts)} completions={len(completions)} expected={expected}. "
            "Use the separately built P5 quicinterop binary."
        )

    for index in range(1, expected + 1):
        if index not in starts or index not in completions:
            raise SystemExit(f"ERROR: missing P5 request marker for index {index}")
        if completions[index]["success"] != 1:
            raise SystemExit(f"ERROR: P5 download {index} failed")
    ...
    if args.gap_us > 0 and len(gaps) != expected_gaps:
        raise SystemExit(
            f"ERROR: observed {len(gaps)}/{expected_gaps} required P5 gap markers"
        )
    ...
    if any(value <= 0 for value in durations_us):
        raise SystemExit("ERROR: at least one P5 download duration is not positive")
    ...
    if workload_elapsed_us <= 0:
        raise SystemExit("ERROR: P5 workload elapsed time is not positive")
    ...
    metrics = {
        "schema": "greenquic-p5-repeated-downloads-v1",
        "mode": args.mode,
        "processes": {"client": 1},
        "quic_connections": 1,
        "streams": expected,
