...
SEQTAG="TOP3_T41_P7_RESUME_${STAMP}"
...
REMOTE_SCRIPT="/tmp/${SEQTAG}_remote.sh"
...
cat <<HDR
======================================================================
RESUME: P5 TOP3 -> P5 T41 -> P7 FAIR
branch=$BRANCH
sha=$SHA
runs=$RUNS downloads=$DOWNLOADS
T29 is NOT rerun; keep the already-complete T29 matrix.
TOP3: PRESSURE_UP=450 RX_QUEUE_HIGH=48 ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16
T41:  ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16
P7: paper Linux profile
matrix folders: TOP3_<timestamp>, T41_<timestamp>, P7_<timestamp>
network handling: wait for Tinyman + retry whole test on SSH/network loss
recording: power/RAPL + frequency 1ms + C-state
chart generation: enabled (chart-style=both)
...
STAMP="$1"; SEQTAG="$2"; SHA="$3"; BRANCH="$4"; RUNS="$5"; DOWNLOADS="$6"; GAP="$7"; EDGE="$8"; BETWEEN="$9"; SEED="${10}"; BUNDLE="${11}"; BUNDLE_REF="${12}"
ROOT=/root/mohsen
...
P5_TOP3_OUT="$P5/matrix_results/TOP3_${STAMP}"
P5_T41_OUT="$P5/matrix_results/T41_${STAMP}"
...
wait_tinyman(){
    ...
    while ! ssh -n -o BatchMode=yes -o ConnectTimeout=5 root@tinyman true >/dev/null 2>&1; do
        n=$((n+1))
        if (( n >= 60 )); then
            echo "ERROR: Tinyman is still unreachable after 10 minutes" >&2
            return 1
        fi
        log "Tinyman unreachable; waiting 10s before retry ($n/60)"
        sleep 10
    done
...
validate_p5_matrix(){
    local label="$1" out="$2" mode
    test -d "$out/runs"
    grep -RFl 'GreenQUIC Run Summary' "$out/runs" >/dev/null
    for mode in off basic plus; do
        find "$out/runs" -type d -name "*_${mode}" -print -quit | grep -q . || {
            echo "ERROR: $label missing mode=$mode run bundles" >&2
            return 1
        }
    done
    test -f "$out/the_sheet_rules_all/GreenQUIC_full_results.xlsx"
    test -d "$out/tables/charts"
    test -d "$out/the_sheet_rules_all"
    log "$label validation PASS: OFF/BASIC/PLUS bundles + charts + workbook"
}
...
run_p5_matrix(){
    local label="$1" out="$2" logfile="$3"
    shift 3
    local extra=("$@") attempt rc
    for attempt in 1 2 3; do
        ...
        set -e
        if (( rc == 0 )); then
            validate_p5_matrix "$label" "$out"
            return 0
        fi
        if is_network_failure "$rc" "$logfile"; then
            log "$label hit SSH/network failure rc=$rc; incomplete matrix will be discarded and retried"
            wait_tinyman
            cleanup_both
            rm -rf "$out"
            sleep 10
        ...
        echo "ERROR: $label failed rc=$rc for a non-network reason" >&2
        return "$rc"
    done
    echo "ERROR: $label failed after 3 network-retry attempts" >&2
    return 1
}

run_p7(){
    local attempt rc logfile="$ART/p7.log"
    for attempt in 1 2 3; do
        wait_tinyman
        cleanup_both
        rm -rf "$P7_OUT"
        ...
        if is_network_failure "$rc" "$logfile"; then
            log "P7 hit SSH/network failure rc=$rc; incomplete matrix will be discarded and retried"
            wait_tinyman
            cleanup_both
            rm -rf "$P7_OUT"
            sleep 10
        ...
        echo "ERROR: P7 failed rc=$rc for a non-network reason" >&2
        return "$rc"
    done
    echo "ERROR: P7 failed after 3 network-retry attempts" >&2
    return 1
}
...
log "wait for Tinyman before sync"
wait_tinyman
...
run_p5_matrix TOP3 "$P5_TOP3_OUT" "$ART/p5_top3.log" \
    --env PRESSURE_UP=450 \
    --env RX_QUEUE_HIGH=48 \
    --env ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16

sleep "$BETWEEN"
log "TEST 2/3 P5 T41 ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16 — OFF + BASIC + PLUS"
run_p5_matrix T41 "$P5_T41_OUT" "$ART/p5_t41.log" \
    --env ACTIVE_TRANSFER_SLEEP_MIN_LEVEL=16
...
P5_t41=ACTIVE_TRANSFER_SLEEP_MIN_LEVEL_16
P5_dpdk_cpu=19
...
TOP3_matrix=$P5_TOP3_OUT
T41_matrix=$P5_T41_OUT
P7_matrix=$P7_OUT
CFG

log "zip resumed result directories"
TOP3_ZIP="/root/TOP3_${STAMP}.zip"
T41_ZIP="/root/T41_${STAMP}.zip"
P7_ZIP="/root/P7_${STAMP}.zip"
(cd "$(dirname "$P5_TOP3_OUT")" && zip -qr "$TOP3_ZIP" "$(basename "$P5_TOP3_OUT")")
(cd "$(dirname "$P5_T41_OUT")" && zip -qr "$T41_ZIP" "$(basename "$P5_T41_OUT")")
(cd "$(dirname "$P7_OUT")" && zip -qr "$P7_ZIP" "$(basename "$P7_OUT")")
printf '%s\n%s\n%s\n' "$TOP3_ZIP" "$T41_ZIP" "$P7_ZIP" > "$ART/RESULT_ZIPS.txt"
touch "$ART/DONE"
log "DONE"
...
BUNDLE_REF="$TMPREF"
ssh idex "rm -rf '$ART'; nohup setsid bash '$REMOTE_SCRIPT' '$STAMP' '$SEQTAG' '$SHA' '$BRANCH' '$RUNS' '$DOWNLOADS' '$GAP' '$EDGE' '$BETWEEN' '$SEED' '$REMOTE_BUNDLE' '$BUNDLE_REF' >'$REMOTE_LOG' 2>&1 </dev/null & echo \$! >'$REMOTE_PID'"
sleep 5

RPID="$(ssh idex "cat '$REMOTE_PID' 2>/dev/null || true")"
if [[ -z "$RPID" ]] || ! ssh idex "kill -0 '$RPID' 2>/dev/null"; then
    echo "ERROR: remote resume sequence died during startup"
...
echo "REMOTE MATRIX NAMES: TOP3_$STAMP  T41_$STAMP  P7_$STAMP"
...
echo "Waiting for TOP3, T41 and P7; Mac must remain awake/network-connected for automatic SCP."
while :; do
    if ssh idex "test -f '$ART/DONE'"; then
        break
    fi
    if ssh idex "test -f '$ART/FAILED'"; then
        echo "REMOTE RESUME SEQUENCE FAILED"
...
echo "Remote resume sequence DONE; downloading result ZIPs..."
