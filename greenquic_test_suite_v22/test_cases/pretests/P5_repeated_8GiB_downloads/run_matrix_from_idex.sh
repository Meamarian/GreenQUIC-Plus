...
#   -> P5 client waits at a start gate (NO GET yet)
#   -> aligned RAPL starts
...
[[ -f "$CORE" ]] || {
    echo "ERROR: missing preserved P5 controller core: $CORE" >&2
    exit 2
}

# P5 result bundling generates C-state SVGs. Fail before any measured workload
# if the plotting dependency is missing instead of wasting a transfer.
python3 -c 'import matplotlib' >/dev/null 2>&1 || {
    echo "ERROR: Python matplotlib is required for P5 result bundling." >&2
    echo "Install python3-matplotlib on both idex and tinyman before running P5." >&2
    exit 2
}
...
# Fresh-install paths.
src = src.replace(
    "/root/greenquic_snapshot/greenquic_test_suite_v22/test_cases/pretests/P5_repeated_8GiB_downloads",
...
needle = '''ssh -n "root@$CLIENT_HOST" "test -x $(quote "$CLIENT_BIN_EFFECTIVE")" || {
    echo "ERROR: separate P5 client binary is missing: $CLIENT_BIN_EFFECTIVE" >&2
    exit 2
}
'''
replacement = needle + '''ssh -n "root@$CLIENT_HOST" "grep -aFq -- GreenQUIC-P5-SEQUENCE-V2 $(quote \"$CLIENT_BIN_EFFECTIVE\")" || {
    echo "ERROR: tinyman P5 binary is not the corrected V2 sequential client: $CLIENT_BIN_EFFECTIVE" >&2
    exit 2
}
...
'''
replace_once(needle, replacement, "P5 binary preflight")
...
# Create a unique remote start gate and pass it to the P5 client. The client
...
'''
new_client_env = '''    client_start_gate="/tmp/p5_start_gate_${run_id}_$$_${TEST_INDEX}"
    ssh -n "root@$CLIENT_HOST" "rm -f $(quote "$client_start_gate")"

    CLIENT_RUN_ENV=(
        "${COMMON_RUN_ENV[@]}"
        "GQ_INTEROP_CLIENT_BIN=$CLIENT_BIN_EFFECTIVE"
        "GQ_INTEROP_P5_START_GATE=$client_start_gate"
        "GQ_INTEROP_P5_GATE_TIMEOUT_US=120000000"
    )
'''
...
new_block = r'''    server_aligned_state="$OUTPUT_DIR/aligned_server_${run_id}.start.json"
    ...
    client_remote_state="/tmp/p5_aligned_client_${run_id}_$$.start.json"
    client_remote_result="/tmp/p5_aligned_client_${run_id}_$$.json"
    ...
    for ((second=0; second<READY_TIMEOUT_SECONDS; second++)); do
        ...
        grep -Fq '[GreenQUIC-P5] ready_for_start_gate_us=' "$client_log" 2>/dev/null && gate_ready=1 || true
        ...
        kill -0 "$SERVER_PGID" 2>/dev/null || {
            echo "ERROR: server exited before P5 readiness" >&2
            stop_server
            exit 1
        }
        kill -0 "$client_pipeline_pid" 2>/dev/null || {
            echo "ERROR: client exited before reaching P5 start gate" >&2
    ...
    [[ "$gate_ready" == 1 ]] || {
        echo "ERROR: Tinyman did not establish QUIC and reach the P5 start gate within ${READY_TIMEOUT_SECONDS}s" >&2
        stop_server
        exit 1
    }
    ...
    if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
        ...
        python3 "$HERE/rapl_window.py" start \
            --state "$server_aligned_state" --label "P5 server connected edge-cooldown window" \
            --role server --mode "$mode" --run-id "$run_id"
        if ssh -n "root@$CLIENT_HOST" \
            "cd $(quote "$CLIENT_DIR") && python3 ./rapl_window.py start --state $(quote "$client_remote_state") --label $(quote 'P5 client connected edge-cooldown window') --role client --mode $(quote "$mode") --run-id $(quote "$run_id")"; then
    ...
    notice "TEST $(printf '%02d' "$TEST_INDEX")/$TOTAL_TESTS mode=$mode releasing P5 start gate; DOWNLOAD 1/$DOWNLOADS may begin"
