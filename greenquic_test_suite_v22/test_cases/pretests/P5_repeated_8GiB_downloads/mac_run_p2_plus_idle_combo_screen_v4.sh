...
TAG="${P5_P2_PLUS_IDLE_TAG:-$(date +%Y%m%d_%H%M%S)}"
SSH_OPTS=(-o ConnectTimeout=15 -o ServerAliveInterval=20 -o ServerAliveCountMax=3)
LOCAL_LOCK="$HOME/Downloads/.greenquic_p2_plus_idle_combo.lock"
BUNDLE="${TMPDIR:-/tmp}/GreenQUIC_P2_PLUS_IDLE_${TAG}.bundle"
REMOTE_BUNDLE="/tmp/GreenQUIC_P2_PLUS_IDLE_${TAG}.bundle"
REMOTE_SCRIPT_LOCAL="${TMPDIR:-/tmp}/P5_P2_PLUS_IDLE_REMOTE_${TAG}_$$.sh"
REMOTE_SCRIPT="/tmp/P5_P2_PLUS_IDLE_REMOTE_${TAG}.sh"
REMOTE_LOG="/root/P5_P2_PLUS_IDLE_${TAG}.log"
REMOTE_PID="/tmp/P5_P2_PLUS_IDLE_${TAG}.pid"
EXPORT_REMOTE="/tmp/P5_P2_PLUS_IDLE_EXPORT_${TAG}"
EXPORT_LOCAL="$HOME/Downloads/P5_P2_PLUS_IDLE_EXPORT_${TAG}"
...
if [[ "${1:-}" == "--detach" ]]; then
    shift
    LOG="$HOME/Downloads/P5_P2_PLUS_IDLE_${TAG}.mac.log"
    PIDFILE="$HOME/Downloads/P5_P2_PLUS_IDLE_${TAG}.mac.pid"
    if [[ -f "$PIDFILE" ]]; then
        old="$(cat "$PIDFILE" 2>/dev/null || true)"
        if [[ -n "$old" ]] && kill -0 "$old" 2>/dev/null; then
            echo "ERROR: PLUS idle combo screen already alive PID=$old" >&2
            exit 70
        fi
    fi
    nohup caffeinate -dimsu env \
        GREENQUIC_REPO="$REPO_ROOT" \
        P5_P2_PLUS_IDLE_TAG="$TAG" \
        P5_P2_RUNS="$RUNS" \
    ...
    echo "STARTED P2 PLUS-IDLE V4 PID=$pid"
    echo "TAG=$TAG"
    echo "LOG=$LOG"
...
if [[ -d "$LOCAL_LOCK" ]]; then
    ...
    if [[ -n "$old" ]] && kill -0 "$old" 2>/dev/null; then
        echo "ERROR: another P2 PLUS-idle screen is alive PID=$old" >&2
        exit 70
    fi
    rm -rf "$LOCAL_LOCK"
fi
...
for pf in \
    ...
    "$HOME/Downloads"/P5_P2_PLUS_IDLE_*.mac.pid; do
    [[ -f "$pf" ]] || continue
    [[ "$pf" == "$HOME/Downloads/P5_P2_PLUS_IDLE_${TAG}.mac.pid" ]] && continue
...
for p in /proc/[0-9]*; do
    ...
    case "$cmd" in *run_matrix_with_sheet.sh*|*.run_matrix_fixed.*|*.run_matrix_plus_*|*run_client.sh*|*run_server.sh*|*run_p5_performance2_*screen.sh*) bad=1;; esac
done
...
fetch_bundle(){
    ...
    REF="refs/heads/__p2_plus_idle_${TAG}_$$"
    ...
    while true; do
        ...
        if (( rc == 255 )); then log "idex -> tinyman SCP transport failed; retry in 30 s"; sleep 30; continue; fi
...
RESULT="/tmp/P5_P2_PLUS_IDLE_COMBO_${TAG}"
MATRIX="$P5/matrix_results/P5_P2_PLUS_IDLE_COMBO_${TAG}"
EX="/tmp/P5_P2_PLUS_IDLE_EXPORT_${TAG}"
rm -rf "$EX"; mkdir -p "$EX"
cd "$P5" || exit 90
STAMP="$TAG" RESULT_ROOT="$RESULT" MATRIX_ROOT="$MATRIX" \
P5_P2_RUNS="$RUNS" P5_P2_DOWNLOADS="$DOWNLOADS" \
bash ./run_p5_performance2_plus_idle_combo_screen.sh
RRC=$?
cp -f "$RESULT/plus_idle_combo_summary.tsv" "$EX/" 2>/dev/null || true
cp -f "$RESULT/plus_idle_combo_vs_baseline.tsv" "$EX/" 2>/dev/null || true
cp -f "$RESULT/status.env" "$EX/" 2>/dev/null || true
cp -f "$SELF_LOG" "$EX/remote.log" 2>/dev/null || true
[ -d "$RESULT" ] && (cd /tmp && zip -qr "$EX/analysis__P5_P2_PLUS_IDLE_${TAG}.zip" "P5_P2_PLUS_IDLE_COMBO_${TAG}")
[ -d "$MATRIX" ] && (cd "$(dirname "$MATRIX")" && zip -qr "$EX/matrix__P5_P2_PLUS_IDLE_${TAG}.zip" "$(basename "$MATRIX")")
printf 'REMOTE_SCRIPT_RC=%s\nP2_SHA=%s\nRUNS=%s\nDOWNLOADS=%s\nMODE=plus\nWORKLOAD=idle_monitor_normal\n' "$RRC" "$SHA" "$RUNS" "$DOWNLOADS" > "$EX/result_rc.txt"
...
wait_remote_done(){
    while true; do
        if ssh_once "test -f '$EXPORT_REMOTE/DONE'" >/dev/null 2>&1; then log "remote PLUS-idle combo screen DONE detected"; return 0; fi
        ...
        log "waiting for PLUS-idle combo screen (Mac->idex SSH failure cannot kill remote nohup job)"
        sleep 60
    done
}
...
log "P2 PLUS-IDLE V4 startup tag=$TAG runs=$RUNS downloads=$DOWNLOADS"
wait_clean_both
sleep "$START_DELAY"
fetch_bundle
sync_branch
...
ssh_transport_retry "cd '/root/mohsen/$P5_REL' && bash -n ./build_p5_performance2.sh ./run_p5_performance2_plus_idle_combo_screen.sh ./run_matrix_from_idex.sh ./run_matrix_from_idex_core.sh && python3 -m py_compile ./apply_p5_performance2.py ./test_p5_performance2_transform.py ./apply_p5_performance2_v2.py ./test_p5_performance2_v2_transform.py && python3 ./test_p5_performance2_transform.py && python3 ./test_p5_performance2_v2_transform.py"
...
cat "$EXPORT_LOCAL/plus_idle_combo_vs_baseline.tsv" 2>/dev/null || true
