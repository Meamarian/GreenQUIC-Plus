...
BATCH_START=re.compile(r'^\[GreenQUIC-PARALLEL\] batch=1 start_us=(\d+) connections=(\d+)$');BATCH_END=re.compile(r'^\[GreenQUIC-PARALLEL\] batch=1 complete_us=(\d+) duration_us=(\d+) connections=(\d+) connected=(\d+) completed=(\d+) success=(0|1)$');CONN_START=re.compile(r'^\[GreenQUIC-PARALLEL\] conn=(\d+)/(\d+) start_us=(\d+) local_port=(\d+) path=(\S+)$');CONN_END=re.compile(r'^\[GreenQUIC-PARALLEL\] conn=(\d+)/(\d+) complete_us=(\d+) duration_us=(\d+) success=(0|1) local_port=(\d+) path=(\S+)$')
def main()->int:
 ap=argparse.ArgumentParser();ap.add_argument('--log',type=Path,required=True);ap.add_argument('--manifest',type=Path,required=True);ap.add_argument('--mode',choices=('off','basic','plus'),required=True);ap.add_argument('--connections',type=int,required=True);ap.add_argument('--out',type=Path,required=True);ap.add_argument('--text-out',type=Path,required=True);ap.add_argument('--goodput-out',type=Path);a=ap.parse_args();text=a.log.read_text(encoding='utf-8',errors='replace');manifest=json.loads(a.manifest.read_text(encoding='utf-8'));starts={};ends={};bs=None;be=None
 ...
 metrics={'schema':'greenquic-p5-parallel-connections-v2','mode':a.mode,'processes':{'client':1},'quic_connections':n,'streams':n,'payload_bytes_per_connection':per_payload,'payload_bytes_total':total_bytes,'local_udp_ports':ports,'batch_start_us':bs[0],'batch_complete_us':int(be[0]),'workload_elapsed_us':duration_us,'aggregate_goodput_gbps':aggregate,'aggregate_goodput_excluding_gaps_gbps':aggregate,'aggregate_goodput_including_gaps_gbps':aggregate,'connection_duration_us':[int(ends[i]['duration_us']) for i in range(1,n+1)],'connection_goodput_gbps':[r['goodput_gbps'] for r in per],'connections':per};a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_text(json.dumps(metrics,indent=2)+'\n',encoding='utf-8')
 ...
 lines += [f'- Average individual goodput: {mean(r["goodput_gbps"] for r in per):.6f} Gbit/s',f'- TOTAL aggregate goodput: {aggregate:.6f} Gbit/s',f'- Aggregate goodput excluding gaps: {aggregate:.6f} Gbit/s',f'- Aggregate goodput including gaps: {aggregate:.6f} Gbit/s',f'- Local UDP ports: {",".join(map(str,ports))}','- Goodput definition: total useful payload bits from all concurrent connections / parallel batch wall-clock duration.','- Do not use quicinterop total execution/transmission time for aggregate goodput.',''];a.text_out.write_text('\n'.join(lines),encoding='utf-8')
 if a.goodput_out:
  gp={'schema':'greenquic-goodput-v4-parallel','test_id':'P5','mode':a.mode,'definition':'total successfully downloaded payload bits across all concurrent QUIC connections divided by parallel batch wall-clock duration','payload_bytes':total_bytes,'payload_gib':total_bytes/(1024**3),'quic_connections':n,'local_udp_ports':ports,'timing_source':'GreenQUIC parallel batch CLOCK_MONOTONIC microsecond markers','primary':{'duration_s':duration_us/1e6,'goodput_bps':aggregate*1e9,'goodput_mbps_decimal':aggregate*1e3,'goodput_gbps_decimal':aggregate},'per_connection':per};a.goodput_out.write_text(json.dumps(gp,indent=2)+'\n',encoding='utf-8')
 print('\n'.join(lines));return 0
