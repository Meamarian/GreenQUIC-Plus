...
def main():
    ...
    for case in sorted(p for p in root.iterdir() if p.is_dir() and re.match(r'^[A-P]_',p.name)):
        ...
        # Prefer numeric analyzer if raw log parsing is unavailable.
        ...
        rows.append({'case':case.name,'n':n,'mean_gbps':mean,'sd_gbps':sd,'max_gbps':max(raw) if raw else mean,'reached_11g':int((max(raw) if raw else mean)>=11.0),'dpdk_lcores':cfg.get('dpdk_lcores',''),'quic_cpus':cfg.get('quic_cpus',''),'affinitize':cfg.get('quic_affinitize',''),'execution_profile':cfg.get('execution_profile',''),'partition_style':cfg.get('partition_style',''),'partition_map':cfg.get('partition_map',''),'build_profile':sr.get('profile',''),'build_rc':sr.get('build_rc',''),'traffic_rc':cfg.get('traffic_rc',sr.get('traffic_rc','')),'controller_rc':cfg.get('controller_rc',sr.get('controller_rc','')),'analysis_rc':cfg.get('analysis_rc',sr.get('analysis_rc','')),'traffic_success_logs':cfg.get('traffic_success_logs',''),'goodput_files':cfg.get('goodput_files',''),'server_active_threads':sa,'client_active_threads':ca,'server_thread_cpu_s':stotal,'client_thread_cpu_s':ctotal,'server_top_thread_cpus':stopcpu,'client_top_thread_cpus':ctopcpu,'server_top_thread_allowed':stopallow,'client_top_thread_allowed':ctopallow})
    ref=next((r for r in rows if r['case'].startswith('B_')),None)
    for r in rows:
        r['delta_vs_B_pct']=''
        if ref and ref['mean_gbps']>0 and r['mean_gbps']>0: r['delta_vs_B_pct']=100*(r['mean_gbps']/ref['mean_gbps']-1)
    ...
    lines=['P5 ARCHITECTURAL BOTTLENECK SWEEP','=================================','Target: find a structural change capable of moving P5 toward >=11 Gbit/s.','']
    for r in rows:
        d='' if r['delta_vs_B_pct']=='' else f" delta_vs_B={r['delta_vs_B_pct']:+.2f}%"
        lines.append(f"{r['case']}: n={r['n']} mean={r['mean_gbps']:.6f} SD={r['sd_gbps']:.6f} max={r['max_gbps']:.6f} Gbit/s{d} build_rc={r['build_rc']} traffic_rc={r['traffic_rc']} controller_rc={r['controller_rc']} analysis_rc={r['analysis_rc']}")
    ranked=sorted((r for r in rows if r['mean_gbps']>0),key=lambda x:x['mean_gbps'],reverse=True)
    lines+=['','RANKING']+[f"{i:02d}. {r['case']} {r['mean_gbps']:.6f} Gbit/s" for i,r in enumerate(ranked,1)]
    if ranked: lines+=['',f"BEST={ranked[0]['case']} {ranked[0]['mean_gbps']:.6f} Gbit/s",f"REACHED_11G={'YES' if ranked[0]['max_gbps']>=11 else 'NO'}"]
