...
while IFS=$'\t' read -r TEST_INDEX rep position mode <&9; do
    ...
    if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
        ...
        python3 "$HERE/rapl_window.py" start \
            --state "$server_aligned_state" --label "P5 aligned server window" \
            --role server --mode "$mode" --run-id "$run_id"
    fi

    client_inner="set -uo pipefail; cd $(quote "$CLIENT_DIR"); client_rc=0; rapl_started=0;"
    if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
        client_inner+=" if python3 ./rapl_window.py start --state $(quote "$client_remote_state") --label $(quote 'P5 aligned client window') --role client --mode $(quote "$mode") --run-id $(quote "$run_id"); then rapl_started=1; else echo '[P5:WARN] client aligned RAPL start failed; continuing with N/A'; fi;"
    fi
    client_inner+=" set +e; env $client_env_words ./run_client.sh; client_rc=\$?; set -e;"
    if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
        client_inner+=" if [[ \$rapl_started == 1 ]]; then python3 ./rapl_window.py finish --state $(quote "$client_remote_state") --out $(quote "$client_remote_result") || echo '[P5:WARN] client aligned RAPL finish failed; continuing with N/A'; fi;"
    fi
