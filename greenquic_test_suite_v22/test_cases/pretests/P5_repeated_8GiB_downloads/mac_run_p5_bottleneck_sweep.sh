...
if [[ "${1:-}" == "--detach" ]]; then
  ...
  echo "P5 only; same ${CONNECTIONS}x8GiB OFF workload in every case; failed experimental cases do not stop later cases."
  exit 0
fi
...
log "installing P5 bottleneck cleanup helper on IDEX + Tinyman"
retry ssh "${SSH[@]}" idex "mkdir -p '$REMOTE_CLEAN_DIR'"
retry scp "${SSH[@]}" "$BASE_CLEANER" "$SWEEP_CLEANER" "idex:$REMOTE_CLEAN_DIR/"
retry ssh "${SSH[@]}" idex "ssh -o BatchMode=yes -o ConnectTimeout=15 root@tinyman 'mkdir -p '$REMOTE_CLEAN_DIR'' && scp -q -o BatchMode=yes -o ConnectTimeout=15 '$REMOTE_CLEAN_DIR/'*.py root@tinyman:'$REMOTE_CLEAN_DIR/'"
...
# Sync exact Mac branch commit to both servers. Preserve all untracked results.
...
# Static preflight only. Build-specific incompatibilities are recorded per case.
retry ssh "${SSH[@]}" idex "cd '$P5' && \
  bash -n ./mac_run_p5_bottleneck_sweep.sh && \
  bash -n ./run_p5_bottleneck_sweep.sh && \
  bash -n ./run_p5_parallel_off_case.sh && \
  bash -n ./build_p5_bottleneck_profile.sh && \
  python3 -m py_compile \
    ./apply_p5_bottleneck_txq.py \
    ./cpu_busy_sampler.py \
...
# Detached remote sweep + packaging. Sweep itself continues across case failures.
...
echo "Packaging all produced case data."
echo "======================================================================"
if [[ -d "$MATRIX" ]]; then
...
# Resumable live remote terminal stream.
: > "$LOCAL_LIVE_LOG"
while true; do
  ...
  if ssh "${SSH[@]}" idex "test -f '$REMOTE_STATE.FAIL'" >/dev/null 2>&1; then
    log "remote packaging/infrastructure state reports FAIL; preserving partial data"
    break
  fi
  ...
  if ! ssh "${SSH[@]}" idex "kill -0 '$REMOTE_PID' 2>/dev/null || test -f '$REMOTE_STATE.DONE' || test -f '$REMOTE_STATE.FAIL'" >/dev/null 2>&1; then
    log "remote PID exited; proceeding to final retrieval"
    break
  fi
done

mkdir -p "$LOCAL_EXPORT"
# Exact remote log overwrites the reconnectable stream copy: this is the file to send.
