...
# Run from macOS. No worktree is created and only small summary/audit files are
# copied back automatically; the complete matrices remain on IDEX.

REPO="${GREENQUIC_REPO:-$(pwd)}"
BRANCH="performance2/p5-multicore"
BASE_BRANCH="performance2/p5-max-goodput"
...
P7REL="greenquic_test_suite_v22/test_cases/pretests/P7_linux_udp_baseline"
PREREL="greenquic_test_suite_v22/test_cases/pretests"
REMOTE_ROOT="/root/mohsen"
P5DIR="$REMOTE_ROOT/$P5REL"
P7DIR="$REMOTE_ROOT/$P7REL"
P5OUT="$P5DIR/matrix_results/P5_PARALLEL_MC_${CONNECTIONS}c_${RUNS}r_${TAG}"
P7OUT="$P7DIR/matrix_results/P7_PARALLEL_MC_${CONNECTIONS}c_${RUNS}r_${TAG}"
P5LOG="/root/P5_PARALLEL_MC_${TAG}.log"
P7LOG="/root/P7_PARALLEL_MC_${TAG}.log"
COMPARE="$REMOTE_ROOT/PARALLEL_MC_COMPARE_${TAG}.csv"
LOCAL_EXPORT="$HOME/Downloads/PARALLEL_MC_FAIR_${TAG}"
mkdir -p "$LOCAL_EXPORT"

cleanup_local(){ rm -f "/tmp/greenquic_parallel_mc_${TAG}.bundle"; }
...
echo "bundle_ref=$BUNDLE_REF size=$(du -h "$BUNDLE" | awk '{print $1}')"
...
CLEAN
    scp -q "$BUNDLE" "$H:/tmp/greenquic_parallel_mc_${TAG}.bundle"
    ssh "$H" bash -s -- "$HEAD" "$BUNDLE_REF" "$TAG" <<'SYNC'
set -Eeuo pipefail
...
git cat-file -e 045d7375c6af5810a9ce30a2db63231989ab3f12^{commit} || {
    echo "ERROR: prerequisite base commit missing on $(hostname)" >&2; exit 71;
}
git fetch "/tmp/greenquic_parallel_mc_${TAG}.bundle" \
    "$BUNDLE_REF:refs/remotes/origin/performance2/p5-multicore"
git checkout -B performance2/p5-multicore refs/remotes/origin/performance2/p5-multicore
git reset --hard "$HEAD"
[[ "$(git rev-parse HEAD)" == "$HEAD" ]] || { echo "ERROR: HEAD mismatch" >&2; exit 72; }
rm -f "/tmp/greenquic_parallel_mc_${TAG}.bundle"
echo "$(hostname): synced HEAD=$(git rev-parse HEAD)"
SYNC
done
...
for H in idex tinyman; do
    echo "----- P5 build $H -----"
    ssh "$H" 'bash -s' <<'P5BUILD'
set -Eeuo pipefail
cd /root/mohsen
DEV=0000:18:00.0
DEVBIND=/root/mohsen/msquic/deps/dpdk/usertools/dpdk-devbind.py
DRIVER="$(basename "$(readlink -f "/sys/bus/pci/devices/$DEV/driver" 2>/dev/null || true)")"
if [[ "$DRIVER" != vfio-pci ]]; then
    modprobe vfio-pci
    python3 "$DEVBIND" -b vfio-pci "$DEV"
fi
DRIVER="$(basename "$(readlink -f "/sys/bus/pci/devices/$DEV/driver" 2>/dev/null || true)")"
[[ "$DRIVER" == vfio-pci ]] || { echo "ERROR: $DEV driver=$DRIVER, expected vfio-pci" >&2; exit 73; }
...
for H in idex tinyman; do
    echo "----- P7 build $H -----"
    ssh "$H" "cd '$P7DIR' && bash ./build_p7_parallel_multicore.sh"
done
...
scp -q "idex:$P5OUT/parallel_tables/parallel_goodput_all_runs.csv" "$LOCAL_EXPORT/P5_parallel_goodput_all_runs.csv"
scp -q "idex:$P5OUT/parallel_tables/parallel_goodput_summary.csv" "$LOCAL_EXPORT/P5_parallel_goodput_summary.csv"
scp -q "idex:$P5OUT/parallel_queue_activity.json" "$LOCAL_EXPORT/P5_parallel_queue_activity.json"
scp -q "idex:$P5OUT/multicore_validation.json" "$LOCAL_EXPORT/P5_multicore_validation.json"
scp -q "idex:$P5OUT/PARALLEL_MULTICORE_CONFIG.txt" "$LOCAL_EXPORT/P5_config.txt"
scp -q "idex:$P7OUT/parallel_tables/parallel_goodput_all_runs.csv" "$LOCAL_EXPORT/P7_parallel_goodput_all_runs.csv"
scp -q "idex:$P7OUT/parallel_tables/parallel_goodput_summary.csv" "$LOCAL_EXPORT/P7_parallel_goodput_summary.csv"
scp -q "idex:$P7OUT/parallel_irq_activity_validation.json" "$LOCAL_EXPORT/P7_parallel_irq_activity_validation.json"
scp -q "idex:$P7OUT/multicore_validation.json" "$LOCAL_EXPORT/P7_multicore_validation.json"
scp -q "idex:$P7OUT/PARALLEL_MULTICORE_CONFIG.txt" "$LOCAL_EXPORT/P7_config.txt"
scp -q "idex:$COMPARE" "$LOCAL_EXPORT/fair_P5_P7_total_goodput_comparison.csv"
printf 'branch=%s\ncommit=%s\nP5_REMOTE=%s\nP7_REMOTE=%s\nP5_LOG=%s\nP7_LOG=%s\n' \
    "$BRANCH" "$HEAD" "$P5OUT" "$P7OUT" "$P5LOG" "$P7LOG" \
    > "$LOCAL_EXPORT/REMOTE_PATHS.txt"

printf '\n======================================================================\n'
printf 'PARALLEL MULTICORE FAIR TEST PASS\n'
printf 'Branch commit: %s\n' "$HEAD"
printf 'Small results on Mac: %s\n' "$LOCAL_EXPORT"
printf 'Full P5 matrix: %s:%s\n' idex "$P5OUT"
printf 'Full P7 matrix: %s:%s\n' idex "$P7OUT"
printf '======================================================================\n'
