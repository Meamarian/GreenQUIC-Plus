...
POWER_ROOT = Path(os.environ.get('P5_RAPL_POWER_ROOT', '/sys/class/powercap'))
...
def discover_domains() -> list[dict[str, Any]]:
    ...
    # sysfs class entries are symlinks. Never use os.walk(..., followlinks=True)
    # here: every sysfs device contains a "subsystem" link back to its class,
    # which creates a recursion cycle and can make discovery run forever.
    # Resolve each class entry once, then walk the real device tree without
    # following any symlink encountered below it.
    resolved_roots: list[Path] = []
    for entry in sorted(POWER_ROOT.glob('intel-rapl:*')):
        try:
            root = entry.resolve(strict=True)
        except OSError:
            continue
        if not root.is_dir() or root in resolved_roots:
            continue
        resolved_roots.append(root)

    # Prefer the shallow package roots and skip class aliases that resolve to a
    # subdirectory already covered by an earlier root.
    selected_roots: list[Path] = []
    for root in sorted(resolved_roots, key=lambda item: (len(item.parts), str(item))):
        if any(root == parent or parent in root.parents for parent in selected_roots):
            continue
    ...
    for root in selected_roots:
        for dirpath, dirnames, filenames in os.walk(root, followlinks=False):
            base = Path(dirpath)
            # Explicitly prune symlinked directories such as subsystem/device.
            dirnames[:] = [
                name for name in dirnames
                if not (base / name).is_symlink()
            ]
            if 'name' not in filenames or 'energy_uj' not in filenames:
                continue

            domain = base
            try:
                resolved = str(domain.resolve(strict=True))
            except OSError:
                continue
            if resolved in seen:
                continue
            ...
            domain_name = read_text(domain / 'name') or domain.name
...
def snapshot(label: str, role: str, mode: str, run_id: str) -> dict[str, Any]:
    return {
        'schema': 'greenquic-p5-aligned-rapl-snapshot-v1',
...
def finish(start: dict[str, Any], end: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        'schema': 'greenquic-p5-aligned-rapl-window-v1',
        'host': start.get('host'),
...
def main() -> int:
    ...
    if args.command == 'start':
        ...
        print(
            f"[P5-ALIGNED-RAPL] START role={args.role} mode={args.mode} "
    ...
    end_data = snapshot(
        str(start_data.get('label', 'P5 aligned RAPL')),
    ...
    print(
        f"[P5-ALIGNED-RAPL] FINISH role={result['role']} mode={result['mode']} "
