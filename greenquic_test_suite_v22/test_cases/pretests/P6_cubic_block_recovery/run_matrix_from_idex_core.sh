...
CLIENT_DIR="${CLIENT_DIR:-/root/greenquic_snapshot/greenquic_test_suite_v22/test_cases/pretests/P5_repeated_8GiB_downloads}"
P5_CLIENT_BIN="${P5_CLIENT_BIN:-/root/greenquic_snapshot/msquic/build-greenquic-p5/bin/Release/quicinterop}"
DOWNLOADS=5
GAP_SECONDS=5
RUNS=5
...
Options:
  ...
  --client-dir PATH            P5 folder on tinyman
  --client-bin PATH            separate P5 quicinterop binary on tinyman
...
while (($#)); do
    case "$1" in
        ...
        --client-bin) P5_CLIENT_BIN="${2:?missing value}"; shift 2 ;;
        --cstate-cpu) P5_CSTATE_CPU="${2:?missing value}"; shift 2 ;;
...
[[ "$P5_CSTATE_CPU" =~ ^[0-9]+$ ]] || { echo "ERROR: --cstate-cpu must be a non-negative integer" >&2; exit 2; }
...
CLIENT_BIN_EFFECTIVE="$(env_value GQ_INTEROP_CLIENT_BIN "$P5_CLIENT_BIN")"
...
ssh -n "root@$CLIENT_HOST" "test -x $(quote "$CLIENT_DIR/run_client.sh")" || {
    echo "ERROR: P5 client folder is missing on $CLIENT_HOST: $CLIENT_DIR" >&2
    exit 2
}
...
# P5-UNIFIED-RESULTS-CPUIDLE-ENV-V1
...
capture_matrix_configuration() {
    ...
    python3 "$HERE/capture_cpuidle_map.py" \
        --cpu "$P5_CSTATE_CPU" --role server \
        --out "$CONFIG_DIR/cpuidle_server.json"

    ssh -n "root@$CLIENT_HOST" \
        "cd $(quote "$CLIENT_DIR") && python3 ./capture_cpuidle_map.py --cpu $(quote "$P5_CSTATE_CPU") --role client" \
...
The P5 run bundles are temporarily created in each endpoint's test results/
...
single P5 result location for this execution.
EOF
}
...
TOTAL_TESTS=$((RUNS * 3))
notice "P5 schedule seed=$SEED strategy=$MODE_ORDER"
...
path.write_text(json.dumps({
    ...
    "aligned_rapl_scope": "pre-cooldown + P5 downloads/gaps + post-cooldown; reporting tail excluded",
}, indent=2) + "\n", encoding="utf-8")
PY
...
while IFS=$'\t' read -r TEST_INDEX rep position mode <&9; do
    ...
    client_result_marker="/tmp/p5_matrix_${run_id}_$$_${TEST_INDEX}.marker"
    : > "$marker"
    ...
    COMMON_RUN_ENV+=(
        ...
        "P5_SERVER_COOLDOWN_SECONDS=$SERVER_COOLDOWN_SECONDS"
    )
    ...
    if [[ "$ALIGNED_RAPL_ENABLED" == 1 ]]; then
        ...
        python3 "$HERE/rapl_window.py" start \
            --state "$server_aligned_state" --label "P5 server edge-cooldown window" \
            --role server --mode "$mode" --run-id "$run_id"
        if ssh -n "root@$CLIENT_HOST" \
            "cd $(quote "$CLIENT_DIR") && python3 ./rapl_window.py start --state $(quote "$client_remote_state") --label $(quote 'P5 client edge-cooldown window') --role client --mode $(quote "$mode") --run-id $(quote "$run_id")"; then
    ...
    final_download_pattern="[GreenQUIC-P5] request=${DOWNLOADS}/${DOWNLOADS} complete_us="
    workload_complete=0
...
echo
notice "SUCCESS: all $COMPLETED_TESTS/$TOTAL_TESTS P5 workloads completed"
