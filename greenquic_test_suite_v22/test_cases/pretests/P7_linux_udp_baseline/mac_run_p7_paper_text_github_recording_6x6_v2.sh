...
# Mac-side P7-only runner for the Linux network configuration described by the
# NOMS 2026 paper + its public tumi8/quic-bypass-paper artifact.
#
# Paper/artifact network knobs:
#   - MsQuic max-throughput profile (provided by P7 config)
#   - 8 GiB object
#   - rmem = 6815744
#   - wmem = 6815744
#   - artifact-style GSO/GRO/checksum/UDP-segmentation activation
#
# GreenQUIC performance tuning deliberately NOT enabled:
#   - no IRQ pinning
#   - no QUIC task pinning
#   - no RPS changes
#   - no combined-channel forcing
#   - no RDMA unbind
#   - no irqbalance manipulation
#   - no D1/D2+ instrumentation
#
# Observation only (does not change the network profile):
#   - RAPL package+DRAM recording, nominal 6 ms
#   - CPU frequency recording on CPU19, nominal 1 ms
#   - CPU19 C-state recording
#
# Deliberate workload-count deviation requested by the experimenter:
#   6 repetitions x 6 sequential 8-GiB downloads.
# MsQuic source/version is whatever the synchronized GreenQUIC branch uses.
...
PATCHED="${TMPDIR:-/tmp}/mac_run_p7_paper_text_github_recording_6x6_${TAG}.sh"
...
# Independent orchestration/export namespace.
src = src.replace('P5_P2_FINAL', 'P7_PAPER_TEXT_GITHUB_RECORDING')
src = src.replace('P2 FINAL', 'P7 PAPER TEXT+GITHUB + RECORDING')
src = src.replace('final P2/P7 runner', 'P7 paper-config recording runner')
src = src.replace('final validation runner', 'P7 paper-config recording runner')
src = src.replace('final 6x6 P5/P7 suite', 'P7 paper-config recording 6x6 suite')

src = src.replace(
    'P7OUT="$P7/matrix_results/P7_FINAL_linux_native_${RUNS}r_${DOWNLOADS}d_${TAG}"',
    'P7OUT="$P7/matrix_results/P7_PAPER_TEXT_GITHUB_RECORDING_${RUNS}r_${DOWNLOADS}d_${TAG}"',
    1,
)

# P5 is unrelated to this P7-only run.
...
src = src[:start] + '''# P5 deliberately skipped: P7 paper-config-only run.\nBP51=0; BP52=0; VP51=0; VP52=0\necho 'P5 SKIPPED (P7 paper-config recording run)'\n\n''' + src[end:]
...
src = src.replace(
    ...
    'zip_one "$P7OUT" "P7_PAPER_TEXT_GITHUB_RECORDING_${RUNS}r_${DOWNLOADS}d_${TAG}"',
    1,
)
