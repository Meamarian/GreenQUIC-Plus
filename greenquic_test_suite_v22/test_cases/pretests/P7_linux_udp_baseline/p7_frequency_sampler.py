...
def bridge(phase: str, attempts: int = 9) -> dict | None:
    ...
    for _ in range(max(1, attempts)):
        ...
        span = raw_after - raw_before
        row = (span, raw_before, raw_after, mono)
    ...
    return {
        'type': 'clock_bridge',
        'schema': 'greenquic-p7-clock-bridge-v1',
        'phase': phase,
...
def main() -> int:
    ...
    interval_ns = max(1, int(args.interval_ms * 1_000_000))
    deadline = start
    with args.output.open('w', encoding='utf-8', buffering=1) as out:
        ...
        while running:
            ...
            elapsed_s = (now - start) / 1e9
            for cpu in cpus:
                khz = read_khz(cpu)
                if khz is not None:
                    out.write(json.dumps({
                        'type': 'line',
                        'schema': 'greenquic-p7-frequency-v1',
                        'monotonic_ns': int(now),
                        'elapsed_s': elapsed_s,
                        'cpu': cpu,
                        'freq_khz': khz,
                    }, separators=(',', ':')) + '\n')
