...
PYFILES=("$P5/clock_sync_parallel.py" "$P5/report_p5_parallel_run.py" "$P5/analyze_p5_parallel_active.py" "$P7/report_p7_parallel_run.py" "$P7/aggregate_p7_parallel_active.py" "$HERE/compare_parallel_p5_p7.py")
for f in "${PYFILES[@]}";do [[ -f "$f" ]] || { echo "ERROR: integration-contract Python file missing: $f" >&2;exit 2; };python3 -m py_compile "$f";done
python3 "$P5/clock_sync_parallel.py" --self-test >/dev/null
...
verify=text(p5/'verify_p5_parallel_multicore_binary.sh');build=text(p5/'build_p5_multicore_performance2.sh');client=text(p5/'run_client_parallel_multicore.sh');p5matrix=text(p5/'run_parallel_multicore_matrix.sh');p5clock=text(p5/'clock_sync_parallel.py');p5report=text(p5/'report_p5_parallel_run.py');p5active=text(p5/'analyze_p5_parallel_active.py');p7build=text(p7/'build_p7_parallel_multicore.sh');p7matrix=text(p7/'run_parallel_multicore_matrix.sh');p7report=text(p7/'report_p7_parallel_run.py');p7active=text(p7/'aggregate_p7_parallel_active.py');macs=text(mac);compare=text(pre/'compare_parallel_p5_p7.py')
...
for needle,label in (('P5-PARALLEL-COMPLETION-V1','parallel completion transform'),('completed=${DOWNLOADS} success=1','successful parallel completion requirement'),('--controller-preflight','P5 nested controller preflight'),('clock_sync_parallel.py','P5 parallel-aware clock sync'),('analyze_p5_parallel_active.py','P5 active analyzer'),('active_window=parallel_batch_start_to_parallel_batch_complete','P5 exact active semantics')):require(p5matrix,needle,label)
for needle,label in (('PARALLEL_FINAL_RE','parallel clock completion regex'),('base.final_download_seen(text)','sequential completion compatibility'),('connected == connections and completed == connections','parallel completion equality check'),('base.spawn_post_sync = spawn_post_sync','parallel drift-audit spawn adapter')):require(p5clock,needle,label)
...
for needle,label in (('sample_monotonic_ns','P5 RAPL monotonic crop'),('client_minus_controller_monotonic_offset_ns','P5 server clock mapping'),('dataplane_mean_ghz','P5 two-CPU frequency aggregate'),("CPUS=(19,20)",'P5 CPU19/20 frequency contract'),('combined_j_per_useful_gbit','P5 active energy efficiency')):require(p5active,needle,label)
...
print('  completion: successful parallel batch marker drives post-cooldown/aligned window and clock drift audit')
