#!/usr/bin/env python3
"""Strict validation for final GreenQUIC V21 dpdk.ini and powermng.ini."""
...
def validate(dpdk_path: Path, power_path: Path, allow_device_placeholder: bool=False):
    ...
    for key in REQUIRED_POWER:
        if key not in power: errors.append(f"missing required final V21 key: {key}")
    ...
    if peer_mac and not re.fullmatch(r"(?:[0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}", peer_mac):
...
def main():
    ...
    if errors: return 2
    print(f"V21 configuration valid: {args.dpdk} + {args.power}"); return 0
