...
load_test() {
    ...
    mkdir -p "$TEST_DIR/logs" "$TEST_DIR/results" "$TEST_DIR/runtime/server" "$TEST_DIR/runtime/client"
}
...
detect_runtime() {
    ...
    if [[ -n "$extra_ld" ]]; then export LD_LIBRARY_PATH="$extra_ld${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"; fi
    if [[ -n "$DETECTED_PKG_CONFIG_PATH" ]]; then export PKG_CONFIG_PATH="$DETECTED_PKG_CONFIG_PATH${PKG_CONFIG_PATH:+:$PKG_CONFIG_PATH}"; fi
    if [[ -z "${GREENQUIC_DPDK_DRIVER_PATH:-}" && -n "$DETECTED_DPDK_DRIVER_PATH" ]]; then
...
verify_msquic_greenquic_config() {
    ...
    [[ -n "$bin" && -x "$bin" ]] || die "No executable runtime binary resolved for role=$role"
    source_root="$(resolve_path "$MSQUIC_DIR")"
    linked_lib="$(linked_msquic_for_binary "$bin" || true)"

    local binary_config_ok=0 multicore_ok=0 plus_ok=0 v18_policy_ok=0 v21_policy_ok=0
    ...
    if [[ "$binary_config_ok" == 0 ]]; then
        if [[ "$ALLOW_UNVERIFIED_MSQUIC_CONFIG" == 1 ]]; then
            warn "The actual runtime binary $bin does not expose the GreenQUIC execution-config markers. Continuing only because ALLOW_UNVERIFIED_MSQUIC_CONFIG=1."
        else
            die "The actual runtime binary $bin does not contain GreenQuicQuicWorkerCpus/GreenQuicQuicProfile markers. Patched source alone is not proof that this binary is current. Rebuild or explicitly set ALLOW_UNVERIFIED_MSQUIC_CONFIG=1 for a non-authoritative run."
        fi
    fi
    if [[ "${ENABLE_MULTICORE:-0}" == 1 && "$multicore_ok" == 0 && "$ALLOW_UNVERIFIED_MSQUIC_CONFIG" != 1 ]]; then
        die "The actual runtime binary/linked libmsquic lacks multicore GreenQUIC markers."
    fi
    if [[ "$requested_mode" == plus && "$plus_ok" == 0 && "$ALLOW_UNVERIFIED_MSQUIC_CONFIG" != 1 ]]; then
        die "The actual runtime binary/linked libmsquic lacks GreenQUIC+ ACK/CUBIC markers."
    fi
    if [[ "$v18_policy_ok" == 0 && "$ALLOW_UNVERIFIED_MSQUIC_CONFIG" != 1 ]]; then
        die "The actual runtime binary/linked libmsquic lacks the final V18 separated-signal policy markers."
    fi
    if [[ "$v21_policy_ok" == 0 && "$ALLOW_UNVERIFIED_MSQUIC_CONFIG" != 1 ]]; then
        die "The actual runtime binary/linked libmsquic lacks the final V21 selectable-idle policy markers."
    fi
...
validate_host_prereqs() {
    ...
    if [[ -n "$expected_host" && -n "$actual_host" && "$actual_host" != "$expected_host" ]]; then
        warn "Expected $role hostname '$expected_host', but this host reports '$actual_host'."
    fi
...
write_dpdk_ini() {
    ...
    mkdir -p "$runtime"
    cat > "$role_config" <<EOF
# Final V21 topology/runtime configuration for $TEST_ID ($role), mode=$mode.
...
write_powermng_ini() {
    ...
    mkdir -p "$runtime"
    cat > "$role_config" <<EOF
# Final GreenQUIC V21 separated-signal and selectable-idle power policy.
...
# V21 selectable idle mechanisms: off | short | pause | monitor | epoll | auto.
...
validate_runtime_config() {
    ...
    python3 "$GQ_COMMON_DIR/bin/validate_v21_config.py" "${args[@]}"
}
...
energy_start() {
    local file="$1" args=(start --out "$file")
...
energy_finish() {
    local start="$1" out="$2" label="$3" args=(finish --start "$start" --out "$out" --label "$label")
    [[ "${GQ_REQUIRE_RAPL:-1}" == 1 ]] && args+=(--require-package)
    python3 "$GQ_COMMON_DIR/bin/energy_meter.py" "${args[@]}"
}
...
run_server() {
    ...
    load_test "$test_dir"
    local mode="${mode_override:-${GQ_MODE_OVERRIDE:-$DEFAULT_MODE}}"
    validate_mode "$mode"
...
client_once() {
    ...
    "$INTEROP_CLIENT_BIN" \
        "-custom:$SERVER_HOST" "-port:$SERVER_PORT" -test:D -serial \
        "-timeout:$CLIENT_TIMEOUT_MS" "-download:$GQ_COMMON_DIR/downloads" \
        "-urls:$url"
}
...
run_inprocess_client() {
    ...
    local args=(
        ...
        --download-dir "$GQ_COMMON_DIR/downloads"
    )
    local p
...
preflight() {
    ...
    log "Preflight passed for actual runtime artifacts, helper requirements, host CPU topology, and strict V21 configuration. NIC binding and EAL startup are verified only when the program is actually run."
}
