#!/usr/bin/env python3
"""Audit every static endpoint configuration with the strict V21 validator."""
from __future__ import annotations
import argparse,importlib.util,sys
from pathlib import Path

def load_validator(path):
    spec=importlib.util.spec_from_file_location('gq_v21_validator',path)
...
def main():
    ...
    root=args.suite_root.resolve(); validator=load_validator(root/'common/bin/validate_v21_config.py'); pairs=[]; errors=[]
    ...
    print(f"V21 suite audit passed: {len(pairs)} strictly validated endpoint configuration pairs"); return 0
