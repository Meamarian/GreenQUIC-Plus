#!/usr/bin/env python3
"""
GreenQUIC autopatcher V22.

V22 ports the complete policy to the active private split Linux DPDK backend
and opts that active translation unit into DPDK 21.11 experimental APIs before
any DPDK header is included, preventing -Werror deprecated-declaration failures.

greenquic_autopatch_v22.py
...
# =============================================================================
# V22 private split-Linux-DPDK backend port
# =============================================================================

V22_VERSION = "22-private-split-linux-dpdk"

_apply_all_patches_v21_2 = apply_all_patches
_build_repo_v21_2 = build_repo


def _gq_between(text: str, begin: str, end: str, *, include: bool = False, start: int = 0) -> str:
    i = text.find(begin, start)
    if i < 0:
        raise RuntimeError(f"Missing begin marker: {begin}")
    j = text.find(end, i + len(begin))
    if j < 0:
        raise RuntimeError(f"Missing end marker after {begin}: {end}")
    if include:
        return text[i:j + len(end)]
    return text[i + len(begin):j]


def _gq_find_function_span(text: str, name: str) -> tuple[int, int]:
    """Return the complete definition span for a C function, excluding prototypes."""
    for match in re.finditer(rf"(?m)^\s*{re.escape(name)}\s*\(", text):
        ...
        while i < len(text):
            ...
            if ch == '"':
                ...
                if depth == 0:
                    ...
                    line_start = text.rfind("\n", 0, match.start()) + 1
                    # Include return type, annotations and static lines immediately above.
                    start = line_start
                    for _ in range(5):
                        prev_end = start - 1
                        if prev_end <= 0:
                        ...
                        if not prev:
                            break
                        if (
                            prev.startswith("_IRQL_") or
                            prev in {"static", "void", "int", "size_t", "QUIC_STATUS", "CXPLAT_SEND_DATA*", "const CXPLAT_INTERFACE*"} or
                            prev.startswith("static ")
                        ):
                            start = prev_start
                        else:
                            break
...
def _gq_replace_function(text: str, name: str, replacement: str) -> str:
    start, end = _gq_find_function_span(text, name)
    return text[:start] + replacement.rstrip() + "\n\n" + text[end:]


def _gq_detect_backend(repo: Path) -> str:
    cmake = read_text(repo / "src/platform/CMakeLists.txt")
    split = "datapath_raw_dpdk_linux.c" in cmake
    legacy = re.search(r"\bdatapath_raw_dpdk\.c\b", cmake) is not None
    if split and not legacy:
        return "split-linux-dpdk"
    if legacy and not split:
        return "legacy-dpdk"
    if split and legacy:
        return "invalid-both-dpdk-backends"
    return "unknown"


_GQ_SPLIT_EXTRA_INCLUDES = r'''
#include <rte_cycles.h>
#include <rte_pause.h>
#include <rte_power.h>
...
#define DEV_RX_OFFLOAD_UDP_CKSUM RTE_ETH_RX_OFFLOAD_UDP_CKSUM
#endif
#ifndef RING_F_MP_HTS_ENQ
#define RING_F_MP_HTS_ENQ 0
#endif
'''


_GQ_SPLIT_PORT_PROTOTYPES = r'''
static BOOLEAN GreenQuicApplyDpdkIniValue(
    _Inout_ DPDK_DATAPATH* Dpdk,
    _In_z_ const char* Key,
    _In_z_ const char* Value);
static void GreenQuicConfigureRoles(
    _Inout_ DPDK_DATAPATH* Dpdk,
    _In_ const struct rte_eth_dev_info* DeviceInfo,
    _Inout_ struct rte_eth_conf* PortConfig,
    _Out_ uint16_t* RxRings,
    _Out_ uint16_t* TxRings);
static int CxPlatDpdkGreenQuicWorkerThread(_In_ void* Context);
'''


_GQ_SPLIT_INI_AND_ROLE_HELPERS = r'''
static BOOLEAN
GreenQuicApplyDpdkIniValue(
    _Inout_ DPDK_DATAPATH* Dpdk,
    _In_z_ const char* Key,
    _In_z_ const char* Value
    )
{
    if (strcmp(Key, "GreenQuicDpdkLcore") == 0) {
        Dpdk->Cpu = (uint16_t)atoi(Value);
        snprintf(
            Dpdk->GreenQuicDpdkLcores,
            sizeof(Dpdk->GreenQuicDpdkLcores),
            "%hu",
            Dpdk->Cpu);
    } else if (strcmp(Key, "GreenQuicDpdkLcores") == 0) {
        snprintf(
            Dpdk->GreenQuicDpdkLcores,
            sizeof(Dpdk->GreenQuicDpdkLcores),
            "%s",
            Value);
    } else if (strcmp(Key, "GreenQuicMode") == 0) {
        if (strcasecmp(Value, "off") == 0 || strcmp(Value, "0") == 0) {
            Dpdk->GreenQuicMode = GREENQUIC_MODE_OFF;
        } else if (
            strcasecmp(Value, "basic") == 0 ||
            strcasecmp(Value, "greenquic") == 0 ||
            strcmp(Value, "1") == 0) {
            Dpdk->GreenQuicMode = GREENQUIC_MODE_BASIC;
        } else if (
    ...
    } else if (strcmp(Key, "GreenQuicEnableMultiCore") == 0) {
        Dpdk->GreenQuicEnableMultiCore = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicTxOwnerLcore") == 0) {
        Dpdk->GreenQuicTxOwnerLcore = (uint16_t)atoi(Value);
        Dpdk->GreenQuicTxOwnerConfigured = TRUE;
    } else if (strcmp(Key, "GreenQuicTxOwnerAlsoRx") == 0) {
        Dpdk->GreenQuicTxOwnerAlsoRx = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicHintLocalityWindowUs") == 0) {
        Dpdk->GreenQuicHintLocalityWindowUs = (uint32_t)atoi(Value);
        if (Dpdk->GreenQuicHintLocalityWindowUs == 0) {
            Dpdk->GreenQuicHintLocalityWindowUs = 1;
        }
    } else if (strcmp(Key, "GreenQuicPartitionDpdkMap") == 0) {
        GreenQuicParsePartitionDpdkMap(Dpdk, Value);
    } else if (strcmp(Key, "GreenQuicEnableRx") == 0) {
        Dpdk->GreenQuicEnableRx = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicEnableTx") == 0) {
        Dpdk->GreenQuicEnableTx = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicEnableFreq") == 0) {
        Dpdk->GreenQuicEnableFreq = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicEnableSleep") == 0) {
        Dpdk->GreenQuicEnableSleep = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicNoSleepIfTxRingNotEmpty") == 0) {
        Dpdk->GreenQuicNoSleepIfTxRingNotEmpty = atoi(Value) != 0 ? TRUE : FALSE;
    } else if (strcmp(Key, "GreenQuicEnableLogging") == 0) {
        Dpdk->GreenQuicLogLevel = atoi(Value) != 0 ? 1U : 0U;
        if (
            Dpdk->GreenQuicLogLevel != 0 &&
            Dpdk->GreenQuicStatsPeriodUs == 0) {
    ...
    } else if (strcmp(Key, "GreenQuicFreqPeriodUs") == 0) {
        Dpdk->GreenQuicFreqPeriodUs = (uint32_t)atoi(Value);
        Dpdk->GreenQuicFreqDownPeriodUs = Dpdk->GreenQuicFreqPeriodUs;
    } else if (GreenQuicApplyPowerConfigValue(Dpdk, Key, Value)) {
        return TRUE;
    } else {
        return FALSE;
    }
    ...
    )
{
    ...
    if (Dpdk->GreenQuicEnableTx) {
        if (
            !Dpdk->GreenQuicTxOwnerConfigured ||
            !rte_lcore_is_enabled(Dpdk->GreenQuicTxOwnerLcore)) {
    ...
    if (Dpdk->GreenQuicEnableMultiCore) {
        const uint16_t MaxRxQueues =
            DeviceInfo->max_rx_queues == 0 ? 1 : DeviceInfo->max_rx_queues;
        uint16_t NextRxQueue = 0;
        unsigned int Lcore;
        ...
        RTE_LCORE_FOREACH(Lcore) {
            const BOOLEAN RequestedRx =
                Dpdk->GreenQuicEnableRx &&
                (!Dpdk->GreenQuicEnableTx ||
                 Lcore != Dpdk->GreenQuicTxOwnerLcore ||
                 Dpdk->GreenQuicTxOwnerAlsoRx);

            if (
                RequestedRx &&
                NextRxQueue < MaxRxQueues &&
                Lcore < RTE_MAX_LCORE) {
                Dpdk->GreenQuicRxQueueByLcore[Lcore] = NextRxQueue++;
            } else if (RequestedRx && NextRxQueue >= MaxRxQueues) {
                printf(
                    "GreenQUIC: lcore %u requested RX but NIC supports only "
                    "%hu RX queues; this lcore receives no RX role.\n",
                    Lcore,
                    MaxRxQueues);
            }
        }
    ...
    printf(
        "GreenQUIC roles: lcores=%u rx_owners=%hu rxq=%hu "
        "tx_enabled=%u tx_owner=%hu tx_owner_also_rx=%u\n",
        rte_lcore_count(),
        Dpdk->GreenQuicRxOwnerCount,
        *RxRings,
        Dpdk->GreenQuicEnableTx ? 1U : 0U,
    ...
    )
{
    ...
    if (Dpdk->GreenQuicMode != GREENQUIC_MODE_OFF) {
        GREENQUIC_LCORE_STATE* State = GreenQuicGetLcoreState(Dpdk, Core);
        if (State->Rx.QueueSampleCountdown == 0) {
            RxQueueCountBefore =
                rte_eth_rx_queue_count(Interface->Port, QueueId);
            State->Rx.QueueSampleCountdown =
                Dpdk->GreenQuicRxQueueSamplePeriod;
        }
        State->Rx.QueueSampleCountdown--;
    }
    ...
    QuicTraceEvent(
        DatapathRxBurst,
        "[data] RX batch received (%u packets).",
        BuffersCount);

    DPDK_RX_PACKET Packet = {0};
    ...
    for (uint16_t Index = 0; Index < BuffersCount; ++Index) {
        ...
        if (likely(Packet.RecvData.Buffer)) {
            ...
            NewPacket->RecvData.DatapathType =
                NewPacket->RecvData.Route->DatapathType =
    ...
    if (likely(PacketCount)) {
        CxPlatDpRawRxEthernet(
            (CXPLAT_DATAPATH_RAW*)Dpdk,
            (CXPLAT_RECV_DATA**)Buffers,
            PacketCount);
    }
}
'''
    ...
    )
{
    ...
    if (unlikely(TxCount < BufferCount)) {
        ...
        QuicTraceEvent(
            LibraryError,
            "[ lib] ERROR, %s.",
            "DPDK: rte_eth_tx_burst() failed to send all packets");
    }
}
'''
    ...
    )
{
    ...
    while (likely(Dpdk->Running)) {
        for (
            Entry = Dpdk->Interfaces.Flink;
            Entry != &Dpdk->Interfaces;
            Entry = Entry->Flink) {
...
static
int
CxPlatDpdkGreenQuicWorkerThread(
    _In_ void* Context
    )
{
    DPDK_DATAPATH* Dpdk = (DPDK_DATAPATH*)Context;
...
def patch_private_split_linux_backend(repo: Path) -> None:
    active_path = repo / "src/platform/datapath_raw_dpdk_linux.c"
    legacy_path = repo / "src/platform/datapath_raw_dpdk.c"
    cmake_path = repo / "src/platform/CMakeLists.txt"
    ensure_file(active_path)
    ensure_file(legacy_path)
    ensure_file(cmake_path)

    backend = _gq_detect_backend(repo)
    if backend != "split-linux-dpdk":
        die(
            "V22 split-backend port expected CMake to select only "
            "datapath_raw_dpdk_linux.c, but detected: " + backend)
    ...
    experimental_opt_in = (
        ...
        '#endif\n')
    if "GREENQUIC-V22-SPLIT-LINUX-DPDK-PORT" in active:
        repaired = insert_after(
            active,
            '#define _CRT_SECURE_NO_WARNINGS 1 // TODO - Remove\n',
            experimental_opt_in,
            "split backend DPDK experimental API opt-in")
        if repaired != active:
            write_text(active_path, repaired)
            log("Repaired missing DPDK experimental API opt-in in the already-ported split backend.")
        else:
            log("Private split Linux DPDK backend is already ported; skipping.")
        return
    ...
    required_active = [
        ...
        'DPDK_INTERFACE Interface; // TODO: support multiple NIC interfaces.',
        'CxPlatDpdkReadConfig(',
        'CxPlatDpdkMainThread',
        'CxPlatDpdkRx(',
        'CxPlatDpdkTx(',
        'CxPlatDpRawSendArpReply(',
        'RawDataPathProcessCqe(',
    ]
    missing = [token for token in required_active if token not in active]
    if missing:
        die(
            "Unsupported private split DPDK backend shape; missing anchors:\n" +
            "\n".join(missing))
    ...
    # The split backend has runtime-configurable burst sizes.
    helpers = helpers.replace("RX_BURST_SIZE", "Dpdk->RxBurstSize")
    helpers = helpers.replace("TX_BURST_SIZE", "Dpdk->TxBurstSize")
    for return_type, function_name in (
        ("uint32_t", "GreenQuicPressureFromRatio"),
        ("uint32_t", "GreenQuicUpdateEwma"),
        ("BOOLEAN", "GreenQuicIsDataPath"),
        ("BOOLEAN", "GreenQuicPlusHasActiveTransferHint"),
    ):
        helpers = re.sub(
            rf"(?m)^static {return_type}\n{function_name}\(",
            f"static {return_type} __attribute__((unused))\n{function_name}(",
            helpers,
            count=1)

    # DPDK 21.11 marks the idle/power APIs used below as experimental.  This
    # opt-in must appear before any DPDK header is included; adding it to the
    # legacy generator file is not sufficient for the active split backend.
    active = insert_after(
        active,
        '#define _CRT_SECURE_NO_WARNINGS 1 // TODO - Remove\n',
        experimental_opt_in,
    ...
    active = insert_after(
        ...
        '#include "datapath_raw.h"\n',
        '#include "greenquic_plus.h"\n',
        "split backend GreenQUIC include")
    active = insert_after(
        active,
        '#include <rte_hexdump.h>\n',
        "\n" + _GQ_SPLIT_EXTRA_INCLUDES + "\n",
        "split backend DPDK/power includes")
    active = insert_after(
        active,
        '#define DEFAULT_SEPARATE_TX_THREAD FALSE\n',
        "\n" + types +
        '\nstatic const char GreenQuicEpollWorkWaitMarker[] '
        '__attribute__((used)) = "epoll_work_wait";\n',
        "split backend GreenQUIC types")
    active = insert_after(
        active,
        '    DPDK_INTERFACE Interface; // TODO: support multiple NIC interfaces.\n',
        "\n    uint16_t Cpu; // GreenQUIC primary DPDK lcore fallback.\n\n" +
        fields + "\n",
        "split backend GreenQUIC state")
    active = insert_after(
        active,
        'static int CxPlatDpdkRxTxWorkerThread(_In_ void* Context);\n',
        "\n" + prototypes + _GQ_SPLIT_PORT_PROTOTYPES + "\n",
        "split backend GreenQUIC prototypes")
    active = insert_before(
        active,
        '_IRQL_requires_max_(PASSIVE_LEVEL)\nvoid\nCxPlatDpdkReadConfig(',
        helpers + "\n" + _GQ_SPLIT_INI_AND_ROLE_HELPERS + "\n",
        "split backend GreenQUIC helpers")
    ...
    active = replace_once(
        ...
        '        Dpdk->Cpu = Config->ProcessorList[0];\n'
        '    }\n',
        "split backend config initialization")
    active = replace_once(
        active,
        '    if (File == NULL) {\n        return;\n    }\n',
        '    if (File == NULL) {\n'
        ...
        '        return;\n'
        '    }\n',
        "split backend config no-file path")
    active = insert_before(
        active,
        '        if (strcmp(Line, "DeviceName") == 0) {\n',
        '        if (GreenQuicApplyDpdkIniValue(Dpdk, Line, Value)) {\n'
        '            continue;\n'
        '        }\n\n',
        "split backend config parser")
    active = replace_once(
        active,
        ...
        '}\n\n_IRQL_requires_max_(PASSIVE_LEVEL)\nsize_t\n'
        'CxPlatDpRawGetDatapathSize(',
        "split backend config finalization")
    ...
    active = replace_once(
        active,
        '                    "TxRing", Dpdk->TxRingSize, '
        'rte_eth_dev_socket_id(Port),\n                    0);',
        '                    "TxRing", Dpdk->TxRingSize, '
        'rte_eth_dev_socket_id(Port),\n'
        '                    RING_F_MP_HTS_ENQ | RING_F_SC_DEQ);',
        "split backend MP/SC TX ring")
    active = insert_after(
        active,
        '    Dpdk->Interface.IfIndex = DeviceInfo.if_index;\n',
        '\n    GreenQuicConfigureRoles(\n'
        '        Dpdk, &DeviceInfo, &PortConfig, &rx_rings, &tx_rings);\n',
        "split backend role/RSS setup")

    old_launch = '''    Dpdk->Running = TRUE;
...
'''
    new_launch = '''    Dpdk->Running = TRUE;
    CxPlatRefInitialize(&Dpdk->RefCount);
    if (Dpdk->GreenQuicEnableMultiCore) {
        ret = rte_eal_mp_remote_launch(
            CxPlatDpdkGreenQuicWorkerThread,
            Dpdk,
            SKIP_MAIN);
        if (ret < 0) {
        ...
        Dpdk->StartStatus = Status;
        CxPlatEventSet(Dpdk->StartComplete);
        CxPlatDpdkGreenQuicWorkerThread(Dpdk);
    } else {
        unsigned int RxLcore = RTE_MAX_LCORE;
        unsigned int TxLcore = RTE_MAX_LCORE;
        if (Dpdk->SeparateRxThread) {
            RxLcore = rte_get_next_lcore(rte_get_main_lcore(), 1, 0);
            if (RxLcore >= RTE_MAX_LCORE) {
                Status = QUIC_STATUS_INVALID_STATE;
                goto Error;
            }
            Dpdk->GreenQuicRxQueueByLcore[RxLcore] = 0;
        ...
        if (Dpdk->SeparateTxThread) {
            TxLcore = rte_get_next_lcore(rte_get_main_lcore(), 1, 0);
            if (Dpdk->SeparateRxThread && TxLcore < RTE_MAX_LCORE) {
                TxLcore = rte_get_next_lcore(TxLcore, 1, 0);
            }
            if (TxLcore >= RTE_MAX_LCORE) {
                Status = QUIC_STATUS_INVALID_STATE;
                goto Error;
            }
            Dpdk->GreenQuicTxOwnerLcore = (uint16_t)TxLcore;
            ret = rte_eal_remote_launch(
                CxPlatDpdkTxWorkerThread,
                Dpdk,
                TxLcore);
        ...
        CxPlatEventSet(Dpdk->StartComplete);
        if (Dpdk->SeparateRxThread && Dpdk->SeparateTxThread) {
            // Main lcore only coordinates the two workers.
        } else if (Dpdk->SeparateRxThread) {
            CxPlatDpdkTxWorkerThread(Dpdk);
        } else if (Dpdk->SeparateTxThread) {
            CxPlatDpdkRxWorkerThread(Dpdk);
        } else {
            CxPlatDpdkRxTxWorkerThread(Dpdk);
        }
    }
...
'''
    active = replace_once(
        ...
        old_launch,
        new_launch,
        "split backend worker launch")

    active = _gq_replace_function(active, "CxPlatDpdkRx", _GQ_RX_FUNCTION)
    active = _gq_replace_function(active, "CxPlatDpdkTx", _GQ_TX_FUNCTION)
    active = _gq_replace_function(
        active,
        "CxPlatDpdkRxWorkerThread",
        _GQ_RX_WORKER)
    active = _gq_replace_function(
    ...
    active = insert_before(
        ...
        'void\nCxPlatDpRawSendArpReply(',
        _GQ_MULTI_WORKER + "\n",
        "split backend multi-core worker")

    active = active.replace(
        'rte_ring_enqueue(Interface->TxRingBuffer, Packet->Mbuf)',
        'rte_ring_mp_enqueue(Interface->TxRingBuffer, Packet->Mbuf)',
        1)
    tx_enqueue_start, tx_enqueue_end = _gq_find_function_span(
        active,
        "CxPlatDpRawTxEnqueue")
    tx_enqueue = active[tx_enqueue_start:tx_enqueue_end]
    if "GreenQuicSignalTxWork(Dpdk);" not in tx_enqueue:
        anchor = '    Dpdk->TxEnqueueCounter++; // increase in any case, even if packet was dropped\n'
        if anchor not in tx_enqueue:
            raise RuntimeError("Could not locate TX enqueue wake anchor")
        tx_enqueue = tx_enqueue.replace(
            anchor,
            '    GreenQuicSignalTxWork(Dpdk);\n' + anchor,
            1)
        active = (
            active[:tx_enqueue_start] + tx_enqueue +
            active[tx_enqueue_end:])

    # Wake monitor/epoll workers before waiting for the DPDK main thread.
    for function_name in ("CxPlatDpRawRelease", "CxPlatDpRawUninitialize"):
        start, end = _gq_find_function_span(active, function_name)
        function = active[start:end]
        if "GreenQuicSignalLcoreWork" not in function:
            if function_name == "CxPlatDpRawRelease":
                anchor = "    force_quit = true;\n"
            else:
                anchor = "    Dpdk->Running = FALSE;\n"
            if anchor not in function:
                raise RuntimeError(
                    f"Could not locate shutdown anchor in {function_name}")
            function = function.replace(
                anchor,
                anchor +
                '    for (uint16_t Core = 0; Core < RTE_MAX_LCORE; ++Core) {\n'
                '        GreenQuicSignalLcoreWork(Dpdk, Core);\n'
                '    }\n',
                1)
            active = active[:start] + function + active[end:]

    # DPDK 21.11 and Clang compatibility.
    active = active.replace(
        "IdleModeEnv[0] != '\\\\0'",
        "IdleModeEnv[0] != '\\0'")
    active = active.replace(
    ...
    write_text(active_path, active)

    cmake = read_text(cmake_path)
    if re.search(r"\bdatapath_raw_dpdk\.c\b", cmake):
        die(
            "V22 refuses to compile the incompatible legacy "
            "datapath_raw_dpdk.c together with the private split backend.")
    if "datapath_raw_dpdk_linux.c" not in cmake:
        die("Private split backend disappeared from CMakeLists.txt.")
    if "greenquic_plus.c" not in cmake:
        cmake = replace_once(
            cmake,
            "set(SOURCES crypt.c hashtable.c pcp.c platform_worker.c toeplitz.c)",
            "set(SOURCES crypt.c hashtable.c pcp.c platform_worker.c "
            "toeplitz.c greenquic_plus.c)",
            "private split backend GreenQUIC+ source")
    write_text(cmake_path, cmake)

    _validate_private_split_backend(repo)
    log(
        "V22 ported GreenQUIC into the active private "
        "datapath_raw_dpdk_linux.c backend without compiling the incompatible "
        "legacy backend.")


def _validate_private_split_backend(repo: Path) -> None:
    active = read_text(repo / "src/platform/datapath_raw_dpdk_linux.c")
    cmake = read_text(repo / "src/platform/CMakeLists.txt")
    required = [
        "GREENQUIC-V22-SPLIT-LINUX-DPDK-PORT",
    ...
    if missing:
        die(
            "V22 active-backend validation failed; missing:\n" +
            "\n".join(missing))
    if re.search(r"\bdatapath_raw_dpdk\.c\b", cmake):
        die("CMake still compiles the incompatible legacy DPDK backend.")
    if "datapath_raw_dpdk_linux.c" not in cmake:
        die("CMake does not compile the active private split DPDK backend.")
    if "greenquic_plus.c" not in cmake:
        die("CMake does not compile greenquic_plus.c.")
    log("V22 active private-backend source validation passed.")


def patch_private_interop_compat(repo: Path) -> None:
    for relative in (
        "src/tools/interopserver/InteropServer.cpp",
        "src/tools/interop/interop.cpp",
    ):
        ...
        text = text.replace(
            ...
            "        ExecConfig->Flags |= QUIC_EXECUTION_CONFIG_FLAG_NONE;")
        text = text.replace(
            "QUIC_EXECUTION_CONFIG_FLAG_AFFINITIZE",
            "QUIC_EXECUTION_CONFIG_FLAG_NONE")
        if relative.endswith("interop.cpp"):
            for token in (
                "uint64_t LastReceiveTime",
                "uint64_t LastReceiveDuration",
                "uint64_t TotalBytesWritten",
            ):
                if token in text and "[[maybe_unused]] " + token not in text:
                    text = text.replace(
                        token,
                        "[[maybe_unused]] " + token,
                        1)
        write_text(path, text)
    log("Applied private-branch interop/Clang compatibility fixes.")


def patch_test_suite_marker_checks(test_suite_dir: Optional[str]) -> None:
    if not test_suite_dir:
        return
    root = Path(test_suite_dir).resolve()
    common = root / "common/bin/gq_common.sh"
    ensure_file(common)
    backup(common)
    text = read_text(common)
    function_start = text.find("verify_msquic_greenquic_config()")
    function_end = text.find("validate_host_prereqs()", function_start)
    if function_start < 0 or function_end < 0:
        die(f"Could not locate marker validation functions in {common}")
    block = text[function_start:function_end]
    replacements = [
        (r'''strings "$bin" 2>/dev/null | grep -q 'GreenQuicQuicWorkerCpus' ''',
         r'''grep -aFq 'GreenQuicQuicWorkerCpus' "$bin" 2>/dev/null '''),
        (r'''strings "$bin" 2>/dev/null | grep -q 'GreenQuicQuicProfile' ''',
         r'''grep -aFq 'GreenQuicQuicProfile' "$bin" 2>/dev/null '''),
        (r'''strings "$artifact" 2>/dev/null | grep -q ''',
         r'''grep -aFq '''),
    ]
    # Exact whole-block replacement is safer across the supplied V21 suite.
    start_marker = (
        "    local binary_config_ok=0 multicore_ok=0 "
        "plus_ok=0 v18_policy_ok=0 v21_policy_ok=0\n")
    end_marker = '    if [[ "$binary_config_ok" == 0 ]]; then\n'
    start = text.find(start_marker, function_start)
    end = text.find(end_marker, start)
    if start < 0 or end < 0:
        die(f"Could not locate marker-check block in {common}")
    replacement = r'''    local binary_config_ok=0 multicore_ok=0 plus_ok=0 v18_policy_ok=0 v21_policy_ok=0

    # Search executable/archive bytes directly. `strings | grep -q` is unsafe
    # under pipefail because grep can close early and make strings exit SIGPIPE.
    ...
    for artifact in "$bin" "$linked_lib"; do
        ...
        if grep -aFq 'CxPlatGreenQuicPlus' "$artifact" 2>/dev/null &&
           grep -aEq 'GQPLUS_HINT_ACK_PENDING|GreenQuicAckClientFloor' "$artifact" 2>/dev/null; then
            plus_ok=1
        fi
        if grep -aFq 'GreenQuicRxBurstRiseAlphaPermille' "$artifact" 2>/dev/null &&
           grep -aFq 'GreenQuicTxRingRiseAlphaPermille' "$artifact" 2>/dev/null &&
           grep -aFq 'GREENQUIC_POWER_CONFIG' "$artifact" 2>/dev/null; then
            v18_policy_ok=1
        fi
...
'''
    text = text[:start] + replacement + text[end:]
    write_text(common, text)
    if shutil.which("bash"):
        p = subprocess.run(["bash", "-n", str(common)])
        if p.returncode != 0:
            die(f"Patched test-suite shell syntax is invalid: {common}")
    log(f"Patched pipefail-safe runtime marker checks: {common}")


def _verify_runtime_markers(binary: Path) -> dict[str, bool]:
    markers = [
    ...
    return {marker: marker.encode() in data for marker in markers}


def _write_v22_manifest(
    repo: Path,
    ...
    import hashlib
    lines = [
        f"GreenQUIC autopatcher version={V22_VERSION}",
        f"repo={repo}",
        f"backend={_gq_detect_backend(repo)}",
        f"dpdk_requested={args.dpdk_checkout}",
        f"enable_multi_core={int(args.enable_multi_core)}",
        f"basic_only={int(args.basic_only)}",
        "trackconnection=0 (unsupported on split backend in V22)",
    ]
    if is_git_repo(repo):
        lines.append(
            "git_commit=" + capture(
    ...
    if shutil.which("clang"):
        lines.append(
            "clang=" + capture(["clang", "--version"], check=False).splitlines()[0])
    if shutil.which("pkg-config"):
        lines.append(
            "libdpdk=" + capture(
                ["pkg-config", "--modversion", "libdpdk"], check=False))
    if build_dir is not None:
        manifest_build_type = args.build_type or "Release"
        for name in ("quicinteropserver", "quicinterop"):
            binary = build_dir / "bin" / manifest_build_type / name
            lines.append(f"binary_{name}={binary}")
            for marker, present in _verify_runtime_markers(binary).items():
                lines.append(
                    f"binary_{name}_marker_{marker}={int(present)}")
    manifest = repo / "greenquic_patch_manifest.txt"
    write_text(manifest, "\n".join(lines) + "\n")
    log(f"Wrote reproducibility manifest: {manifest}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "GreenQUIC V22 complete private-branch autopatcher: ports the "
            "V21.2 policy into the active split Linux DPDK backend"))
    p.add_argument("--git-url", default=None)
    ...
    p.add_argument(
        "--dpdk-mode",
        choices=["local", "system", "windows"],
        default="local")
    p.add_argument("--dpdk-dir", default=None)
    p.add_argument("--dpdk-install-dir", default=None)
    p.add_argument("--dpdk-search-root", default=None)
    p.add_argument(
        "--dpdk-git-url",
        default="https://github.com/DPDK/dpdk.git")
    p.add_argument(
        "--dpdk-checkout",
        default=RECOMMENDED_DPDK_CHECKOUT)
    p.add_argument("--force-dpdk-build", action="store_true")
    p.add_argument("--no-dpdk-build", action="store_true")
    p.add_argument("--enable-example-logging", action="store_true")
    p.add_argument("--enable-multi-core", action="store_true")
    p.add_argument(
        "--trackconnection",
        "--track-connection",
        dest="trackconnection",
        action="store_true",
        help=(
            "Not supported for the private split backend in V22. The "
            "normal partition/lcore hint mapping remains available."))
    p.add_argument(
        "--test-suite-dir",
        default=None,
        help=(
            "Optional GreenQUIC test-suite root. Patches its pipefail-safe "
            "runtime binary marker checks."))
    return p.parse_args()
...
def apply_all_patches(
    ...
    backend_before = _gq_detect_backend(repo)
    if backend_before == "invalid-both-dpdk-backends":
        die(
            "Both DPDK backends are active. Restore the original private "
            "CMakeLists.txt before running V22.")
    if backend_before == "split-linux-dpdk" and track_connection:
        die(
            "--trackconnection is not yet supported on the private split "
            "Linux DPDK backend. Run without it; partition-to-lcore mapping "
            "is fully supported.")

    _apply_all_patches_v21_2(
        repo,
        basic_only=basic_only,
        enable_multi_core=enable_multi_core,
        track_connection=track_connection)

    if backend_before == "split-linux-dpdk":
        patch_private_split_linux_backend(repo)
        patch_private_interop_compat(repo)
    elif backend_before == "legacy-dpdk":
        warn(
            "Legacy backend detected; V22 used the existing complete V21.2 "
            "path without the private split-backend port.")
    else:
        die("Unsupported DPDK backend layout: " + backend_before)


def build_repo(repo: Path, args: argparse.Namespace) -> None:
    build_dir = Path(args.build_dir or "build-greenquic")
    if not build_dir.is_absolute():
        build_dir = repo / build_dir
    build_type = args.build_type or "Release"
    tls = args.tls or "openssl"
    jobs = str(args.jobs or os.cpu_count() or 4)

    log("Configuring local private-branch build.")
    run([
        "cmake", "-S", ".", "-B", str(build_dir),
        f"-DCMAKE_BUILD_TYPE={build_type}",
        f"-DQUIC_TLS={tls}",
    ...
    log("Building quicinteropserver and quicinterop.")
    run([
    ...
    active_object = (
        build_dir / "src/platform/CMakeFiles/msquic_platform.dir" /
        "datapath_raw_dpdk_linux.c.o")
    if _gq_detect_backend(repo) == "split-linux-dpdk" and not active_object.exists():
        die(f"Active split DPDK object was not produced: {active_object}")

    failures: list[str] = []
    for name in ("quicinteropserver", "quicinterop"):
        binary = build_dir / "bin" / build_type / name
        if not binary.exists():
            failures.append(f"missing binary: {binary}")
            continue
        results = _verify_runtime_markers(binary)
        missing = [marker for marker, present in results.items() if not present]
        if missing:
            failures.append(f"{binary}: missing markers {missing}")
    if failures:
        die("Final runtime verification failed:\n" + "\n".join(failures))

    log("Both runtime binaries contain all GreenQUIC V22 markers.")
    _write_v22_manifest(repo, args, build_dir)
...
def main() -> None:
    ...
    if args.trackconnection:
        die(
            "V22 private split-backend build does not support "
            "--trackconnection. Remove that option.")
    for tool in ("git", "cmake"):
        if shutil.which(tool) is None:
            die(f"Required tool missing from PATH: {tool}")

    print(f"""
GreenQUIC full autopatcher V22

This version keeps the complete V21.2 GreenQUIC/GreenQUIC+ implementation, but
correctly ports it into the private branch's active split Linux backend:
  src/platform/datapath_raw_dpdk_linux.c

It never compiles the incompatible legacy datapath_raw_dpdk.c beside that
backend. It preserves the private branch's ARP reply, CQE, reference counting,
separate RX/TX pools and cleanup path, while adding selectable idle modes,
DVFS, BASIC/PLUS pressure, RSS queues, configurable lcores and one TX owner.
""")
    ...
    check_expected_files(repo)
    backend = _gq_detect_backend(repo)
    log(f"Detected DPDK backend: {backend}")

    if not args.yes:
        print("\nThe script will edit files and create .greenquic.bak backups.")
        if not ask_yes("Apply GreenQUIC V22 now?", default=False):
            die("Stopped before patching.")
    ...
    patch_test_suite_marker_checks(args.test_suite_dir)
    _write_v22_manifest(repo, args)
    ...
    print_test_guide(repo)
    print("\nV22 completed. See greenquic_patch_manifest.txt in the repo.")
