...
.PARAMETER UseDpdk
    Enables DPDK support (Linux-only).
...
if ($UseXdp -And $UseDpdk) {
    Write-Error "[$(Get-Date)] Cannot use both XDP and DPDK"
    exit
}
...
function CMake-Generate {
    ...
    if ($UseXdp) {
        ...
        $Arguments += " -DQUIC_LINUX_DPDK_ENABLED=off"
    }
    if ($UseDpdk) {
        $Arguments += " -DQUIC_LINUX_DPDK_ENABLED=on"
        $Arguments += " -DQUIC_LINUX_XDP_ENABLED=off"
    }
    if ($Platform -eq "uwp") {
