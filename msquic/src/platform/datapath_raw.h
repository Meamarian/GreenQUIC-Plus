...
typedef struct CXPLAT_INTERFACE {
    ...
    uint8_t PeerPhysicalAddress[ETH_MAC_ADDR_LEN]; // For ARP/NS workaround
    struct {
        struct {
...
//
// Sends an ARP reply to the sender, as spscified in the ARP header.
// TODO: currently this is just a mock ARP implementation for DPDK
//
void
CxPlatDpRawSendArpReply(
    _In_ const CXPLAT_DATAPATH* Datapath,
    _In_ const ARP_HEADER* ARP
    );
...
typedef struct ARP_HEADER {
    ...
    uint16_t Operation;
    uint8_t SenderHardwareAddr[ETH_MAC_ADDR_LEN];
    uint32_t SenderProtocolAddr;
    uint8_t TargetHardwareAddr[ETH_MAC_ADDR_LEN];
...
#define ARP_OPERATION_REPLY        0x0200
