...
{
    if (Length < sizeof(ARP_HEADER)) {
        QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
                Datapath,
                Length,
                "packet is too small for an ARP header");
        return;
    }

    if (ARP->HardwareType != ARP_HARDWARE_TYPE_ETHERNET) {
        QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
                Datapath,
                ARP->HardwareType,
                "unsupported ARP hardware type");
        return;
    }

    if (ARP->ProtocolType != ETHERNET_TYPE_IPV4) {
        QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
                Datapath,
                ARP->ProtocolType,
                "unsupported ARP protocol type");
        return;
    }

    if (ARP->HardwareAddrLength != sizeof(ARP->SenderHardwareAddr)) {
        QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
                Datapath,
                ARP->HardwareAddrLength,
                "unsupported ARP hardware address length");
        return;
    }

    if (ARP->ProtocolAddrLength != sizeof(ARP->SenderProtocolAddr)) {
        QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
                Datapath,
                ARP->ProtocolAddrLength,
                "unsupported ARP protocol address length");
        return;
    }
    ...
    //
    // TODO: this is just a mock, sending a reply with the configured local MAC if the peer sent the request
    //

    const CXPLAT_INTERFACE* Interface = CxPlatDpRawGetInterfaceFromQueue(Packet->Route->Queue);
    if (memcmp(ARP->SenderHardwareAddr, Interface->PeerPhysicalAddress, ETH_MAC_ADDR_LEN) != 0) {
        QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
                Datapath,
                0,
                "ARP request not from peer");
        return;
    }

    CxPlatDpRawSendArpReply(Datapath, ARP);
}
    ...
    )
{
    ...
    if (IsEthernetBroadcast(Ethernet->Destination) || IsEthernetMulticast(Ethernet->Destination)) {
        if (Ethernet->Type == ETHERNET_TYPE_ARP) {
            CxPlatDpRawParseAndHandleArp(Datapath, Packet, (ARP_HEADER*)Ethernet->Data, Length);
        } else {
            QuicTraceEvent(
                    DatapathErrorStatus,
                    "[data][%p] ERROR, %u, %s.",
                    Datapath,
                    0,
                    "not a unicast packet and not ARP");
            return;
        }
        return;
    }
    ...
    } else if (EthernetType == ETHERNET_TYPE_ARP) {
        CxPlatDpRawParseAndHandleArp(Datapath, Packet, (ARP_HEADER*)Ethernet->Data, Length);
    } else {
        QuicTraceEvent(
            DatapathErrorStatus,
    ...
    )
{
    ...
    // set local port to a fixed ephemeral port if it is 0
    // TODO: this is a q&d fix
    if (Socket->LocalAddress.Ip.sa_family == AF_INET && Socket->LocalAddress.Ipv4.sin_port == 0) {
        Socket->LocalAddress.Ipv4.sin_port = htons(56789);
    } else if (Socket->LocalAddress.Ip.sa_family == AF_INET6 && Socket->LocalAddress.Ipv6.sin6_port == 0) {
        Socket->LocalAddress.Ipv6.sin6_port = htons(56789);
    }
