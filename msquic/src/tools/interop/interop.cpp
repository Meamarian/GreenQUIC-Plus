...
PrintUsage()
{
    ...
    printf("  quicinterop.exe [-target:<implementation> | -custom:<hostname>] [-port:<####>] [-test:<test case>] [-timeout:<milliseconds>] [-download:<path>] [-version:<####>]\n\n");
...
class InteropStream {
    ...
    [[maybe_unused]] uint64_t LastReceiveTime;
        ...
        case QUIC_STREAM_EVENT_RECEIVE:
            if (CustomUrlPath) {
                if (pThis->File == nullptr) {
                    ...
                    std::string dir(DownloadDirectory);
                    std::string file(pThis->FileName);
                ...
                [[maybe_unused]] uint64_t TotalBytesWritten = 0;
    ...
    )
{
    ...
    QUIC_STATUS Status;
    const QUIC_REGISTRATION_CONFIG RegConfig = { "quicinterop", GreenQuicGetQuicExecutionProfile() };
    ...
    if (!TryGetValue(argc, argv, "download", &DownloadDirectory)) {
        DownloadDirectory = ".";
        printf("No Download directory: defaults to %s", DownloadDirectory);
    }
    ...
    pre_ts = get_timestamp_us();

    RunInteropTests(EndpointIndex);

    post_ts = get_timestamp_us();
    printf("transmission time [us]: %lu\n", post_ts - pre_ts);

    if (TryGetValue(argc, argv, "logdir", &LogDirectory)) {
        std::string timefilepath = std::string(LogDirectory) + "/time.json";
        std::ofstream timefile(timefilepath);
        if (timefile) {
            timefile << "{\"start\": " << (pre_ts * 1000UL)
                     << ", \"end\": " << (post_ts * 1000UL) << "}\n";
        } else {
            perror("Failed to open time.json");
        }
    }
