    ...
    )
{
    ...
    *NewConnection = NULL;
    ...
    if (IsServer) {
        ...
        //
        // If dpdk.ini is configured, get the Local IP from that file.
        // Note that the IP address must be configured on some interface of the host,
        // otherwise binding the auxiliary socket will lead to EADDRNOTAVAIL.
        //
        // TODO: this is only a q&d fix for the DPDK datapath!
        //
        FILE* File = fopen("dpdk.ini", "r");
        if (File != NULL) {
            printf("using dpdk.ini to set local IP!\n");
            ...
            while (fgets(Line, sizeof(Line), File) != NULL) {
                ...
                *Value++ = '\0';
                ...
                if (strcmp(Line, "LocalIp") == 0) {
                    QuicAddrFromString(Value, 0, &Path->Route.LocalAddress);
                    Connection->State.LocalAddressSet = TRUE;
