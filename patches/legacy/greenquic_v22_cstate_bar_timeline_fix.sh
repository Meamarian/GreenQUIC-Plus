...
# GreenQUIC V22 C-state bar timeline repair
#
# Apply the same script on idex and tinyman.
#
# Timeline behavior:
#   CSTATE_COMPACT=1
#       One vertical colored bar per completed idle interval.
#
#   CSTATE_COMPACT=500
#       Every 500 completed idle intervals are combined into one compact block:
#         - one stacked colored bar: total duration in every C-state;
#         - one adjacent black bar: wakeup / idle-exit count;
#         - a box above the bar naming the block and listing exactly what it
#           contains, including interval counts per C-state, wakeups, and total
#           idle duration.
#
# The x-axis remains the real kernel-trace time axis. MaxNLocator is used
# instead of a fixed MultipleLocator, so MAXTICKS warnings are avoided.
...
BACKUP="${TARGET}.before_bar_timeline_${STAMP}"
cp -a "$TARGET" "$BACKUP"
...
def compact_annotation(block: CompactBlock, cpu: int) -> str:
    title = (
        f"Block {block.number} — {len(block.intervals)} idle intervals"
    )

    state_parts = [
        f"{state_title(cpu, state)}: {block.count_by_state[state]}"
        for state in sorted(block.count_by_state)
    ]

    state_lines = [
        " | ".join(group)
        for group in chunked(state_parts, 3)
    ]

    total_ms = block.total_idle_ns / 1_000_000.0
    footer = f"Idle total: {total_ms:.3f} ms | Wakeups: {block.wakeups}"

    return "\n".join([title, *state_lines, footer])
...
def timeline_plot(
    *,
    ...
    axes_list = [axes[index][0] for index in range(len(cpus))]
    ...
    for panel_index, cpu in enumerate(cpus):
        ...
        # The block stays on its actual kernel-trace time span. Its colored
        # duration bar and black wakeup bar are adjacent inside that span.
        for block_index, block in enumerate(blocks):
            ...
            state_width = max(span_ms * 0.64, min_visible_width_ms)
            wake_width = max(span_ms * 0.18, min_visible_width_ms)
            state_center = center_ms - span_ms * 0.10
            wake_center = center_ms + span_ms * 0.34
            ...
            max_idle_ms = max(max_idle_ms, bottom_ms)
        ...
        # Put a title box above each compact bar. CSTATE_COMPACT=1 normally
        # suppresses these boxes because every single interval already has its
        # state represented directly by its color.
        if compact_size > 1 and blocks:
            requested_stride = max(1, annotate_every)
            automatic_stride = max(
                1,
                math.ceil(len(blocks) / max(1, max_annotations)),
            )
            ...
            y_padding = max(max_idle_ms * 0.04, 0.001)

            for block_index, block in enumerate(blocks):
                is_last = block_index == len(blocks) - 1
                if block_index % annotation_stride != 0 and not is_last:
                    continue

                start_ms = block.start_ns / 1_000_000.0
                end_ms = block.end_ns / 1_000_000.0
                center_ms = (start_ms + end_ms) / 2.0
                top_ms = block.total_idle_ns / 1_000_000.0

                ax.text(
                    center_ms,
                    top_ms + y_padding,
                    compact_annotation(block, cpu),
                    ha="center",
                    va="bottom",
                    fontsize=9,
                    rotation=0,
            ...
            ax.set_ylim(
                0.0,
                max(max_idle_ms * 1.42, max_idle_ms + y_padding * 8.0),
            )
        elif max_idle_ms > 0.0:
            ax.set_ylim(0.0, max_idle_ms * 1.10)

        ax.set_ylabel(
            "Idle duration in bar (ms)",
            fontsize=14,
        )
        wake_ax.set_ylabel(
            "Wakeups in bar",
        ...
        wake_ax.yaxis.set_major_locator(MaxNLocator(integer=True, nbins=8))

        ax.set_title(
            (
                f"CPU {cpu}: colored C-state bars over kernel-trace time "
                f"— role={role} — compact={compact_size} intervals/bar"
            ),
            fontsize=17,
        )
        ...
        wake_ax.tick_params(axis="y", labelsize=11)
    ...
    fig.suptitle(
        (
            "GreenQUIC Linux C-state bar timeline\n"
            "Stacked colors = time spent in each C-state; "
            "black adjacent bar = wakeup/idle-exit count"
        ),
        fontsize=20,
        y=0.999,
    )
    ...
    return {
        ...
        "timeline_wakeup_bar_unit": "wakeup / idle-exit count",
    }
...
PY
then
    echo "ERROR: Python matplotlib is missing." >&2
    echo "Install it with: apt-get update && apt-get install -y python3-matplotlib" >&2
    exit 1
fi
...
echo "  $BACKUP"
echo
echo "PASS: C-state timeline now uses vertical bars on the real time axis."
echo
echo "Default:"
echo "  CSTATE_COMPACT=500"
echo
echo "Meaning:"
echo "  CSTATE_COMPACT=1"
echo "      one colored bar per completed idle interval"
echo
echo "  CSTATE_COMPACT=500"
echo "      one compact stacked bar per 500 completed idle intervals"
echo "      plus one adjacent black wakeup-count bar"
echo "      plus a title box above the compact bar containing:"
echo "        - block number"
echo "        - number of idle intervals"
echo "        - count of every included C-state"
echo "        - total idle duration"
echo "        - wakeup count"
echo
