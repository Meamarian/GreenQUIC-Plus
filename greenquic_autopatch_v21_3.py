...
# =============================================================================
# V21.3 private split-Linux-DPDK backend port
# =============================================================================

V21_3_VERSION = "21.3-private-split-linux-dpdk"
...
def patch_private_split_linux_backend(repo: Path) -> None:
    ...
    if backend != "split-linux-dpdk":
        die(
            "V21.3 split-backend port expected CMake to select only "
    ...
    if "GREENQUIC-V21.3-SPLIT-LINUX-DPDK-PORT" in active:
...
'''
    ...
    log(
        "V21.3 ported GreenQUIC into the active private "
...
def _validate_private_split_backend(repo: Path) -> None:
    ...
    if missing:
        die(
            "V21.3 active-backend validation failed; missing:\n" +
            "\n".join(missing))
    if re.search(r"\bdatapath_raw_dpdk\.c\b", cmake):
    ...
    log("V21.3 active private-backend source validation passed.")
...
def _write_v21_3_manifest(
    ...
    import hashlib
    lines = [
        f"GreenQUIC autopatcher version={V21_3_VERSION}",
        f"repo={repo}",
        ...
        "trackconnection=0 (unsupported on split backend in V21.3)",
    ]
    if is_git_repo(repo):
        lines.append(
            "git_commit=" + capture(
    ...
    if build_dir is not None:
        for name in ("quicinteropserver", "quicinterop"):
            binary = build_dir / "bin/Release" / name
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "GreenQUIC V21.3 complete private-branch autopatcher: ports the "
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Not supported for the private split backend in V21.3. The "
...
def apply_all_patches(
    ...
    if backend_before == "invalid-both-dpdk-backends":
        die(
            ...
            "CMakeLists.txt before running V21.3.")
    ...
    if backend_before == "split-linux-dpdk":
        ...
        warn(
            "Legacy backend detected; V21.3 used the existing complete V21.2 "
...
def build_repo(repo: Path, args: argparse.Namespace) -> None:
    ...
    log("Both runtime binaries contain all GreenQUIC V21.3 markers.")
    _write_v21_3_manifest(repo, args, build_dir)
...
def main() -> None:
    ...
    if args.trackconnection:
        die(
            "V21.3 private split-backend build does not support "
    ...
    print(f"""
GreenQUIC full autopatcher V21.3
...
""")
    ...
    if not args.yes:
        ...
        if not ask_yes("Apply GreenQUIC V21.3 now?", default=False):
            die("Stopped before patching.")
    ...
    _write_v21_3_manifest(repo, args)
    ...
    print_test_guide(repo)
    print("\nV21.3 completed. See greenquic_patch_manifest.txt in the repo.")
